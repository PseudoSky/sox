...
Regenerated 2026-08-06: **88 open** (BL-478 filed — the render-blocker's durable git-committed record, dual-grammar checker design proposed but not implemented, see its own entry; BL-476 filed — cross-repo `@adhd/backlog` id-allocator defect (`computeNextHumanId` scans only the graph, minted a colliding `BL-437`), filed here per owner direction, see its own entry; BL-447 resolved — structural CHECK-presence gate replaces the DDL substring probe, unblocking the open-typing group, see CHANGELOG.md; BL-475 filed — allocating a BL-id from a worktree leaves the MAIN checkout dirty and failing `check-backlog-markers`, blocking the shared pre-commit hook and aborting merges into `main`, see its own entry; BL-474 filed — `clustering-e2e.test.ts` step 3 races the always-on drain chain for the shared `_bgSlot` mutex, exonerating the concurrent BL-472 merge by code-path analysis, see its own entry; merged with `main`, BL-473 filed — BL-472 resolved 2026-08-06 — shutdown now bounds and drains in-flight Phase-B embeds ahead of `terminateEmbedWorkers()` instead of discarding them, watched red→green across all five acceptance criteria including a genuine end-to-end reproduction; BL-437 resolved 2026-08-06; BL-426 resolved 2026-08-06; BL-416, BL-446, BL-454 resolved 2026-08-06 from PKT-75 — shared BACKLOG.md id-allocation/lint tooling corrected per the owner's shared-registry ruling, see CHANGELOG.md).

| Priority | Open items |
|---|---|
| **CRITICAL** | _(none)_ |
| **HIGH** | BL-225, BL-284, BL-288, BL-301, BL-302, BL-319, BL-322, BL-326, BL-327, BL-334, BL-342, BL-345, BL-349, BL-351, BL-353, BL-356, BL-358, BL-375, BL-387, BL-393, BL-401, BL-404, BL-409, BL-413, BL-422, BL-438, BL-439, BL-440, BL-441, BL-442, BL-448, BL-468, BL-476 |
| **MEDIUM** | BL-99, BL-104, BL-105, BL-228, BL-274, BL-282, BL-285, BL-291, BL-296, BL-306, BL-307, BL-308, BL-312, BL-315, BL-317, BL-318, BL-328, BL-332, BL-333, BL-378, BL-389, BL-396, BL-398, BL-400, BL-443, BL-444, BL-450, BL-452, BL-460, BL-462, BL-466, BL-467, BL-470, BL-474, BL-475 |
| **LOW** | BL-103, BL-202, BL-215, BL-258, BL-261, BL-283, BL-292, BL-298, BL-305, BL-309, BL-314, BL-355, BL-392, BL-408, BL-424, BL-436, BL-455, BL-473, BL-478 |
| **UNSET** | BL-163 |
...
### BL-473 — a second, unowned derived table sits under `## Audit` (`BACKLOG.md:44-45`) and nothing regenerates it, unlike the priority table above it — **Open (LOW, hygiene)** (2026-08-06)

**What's wrong.** `BACKLOG.md:44-45` carries a second `| **LOW** | ... |` / `| **FEATURE** | ... |`
priority-style table, physically sitting under the `## Audit` heading (`BACKLOG.md:36`) though it has
no textual connection to the filesystem-write audit that heading actually describes. Discovered
2026-08-06 while regenerating the *first* priority table (`BACKLOG.md:28-34`, PKT-75 close-out) for
the same root defect — a hand-maintained status table going stale — but this second table is the more
interesting case: the first one at least had a de-facto regenerator (the `node -e` one-liner at
`BACKLOG.md:20-23`, plus `check-backlog-markers.mjs` Rule 5 for its sibling `Total open:` annotation)
and was merely stale. This one has **no regenerator, no check, and no comment identifying which tool
(if any) ever produced it** — so it will go stale again silently after every close-out, indefinitely,
regardless of whether the table above it stays correct.

**Verified dead — three of its fourteen `LOW` ids and both `FEATURE` ids checked, zero have a live
heading:**
- `BL-255` — `grep -c '^### BL-255' BACKLOG.md` → 0. No heading anywhere in the file.
- `BL-289` — `grep -c '^### BL-289' BACKLOG.md` → 0. No heading anywhere in the file.
- `BL-299` — `grep -c '^### BL-299' BACKLOG.md` → 0. No heading anywhere in the file.
- `BL-163` (its `FEATURE` bucket) — heading exists at `BACKLOG.md:117` but the *current*, correctly
  regenerated priority table (`BACKLOG.md:34`) already classifies it under `**UNSET**`, not
  `**FEATURE**` — this table's own bucketing scheme has drifted from the live convention, not just its
  membership.
- `BL-215` (its other `FEATURE` id) — heading exists at `BACKLOG.md:79` and the live table
  (`BACKLOG.md:31`) already carries it correctly under `**LOW**`. Fully redundant with the table above
  it, using a stale label.

The remaining ten `LOW` ids (`BL-103`, `BL-202`, `BL-258`, `BL-261`, `BL-283`, `BL-290`, `BL-292`,
`BL-298`, `BL-305`, `BL-309`, `BL-314`) were not individually re-verified in this pass, but every one
of them is either already present (correctly) in the live priority table (`BACKLOG.md:28-34`) or,
like `BL-255`/`BL-289`/`BL-299`, has no heading at all — either way this second table adds no
information the regenerated one above it doesn't already carry, and carries dead ids the live one
correctly omits.

**Recommendation: delete it.** It predates the current priority-table convention (`BACKLOG.md:14-23`,
BL-224), duplicates a subset of what the regenerated table above it already states correctly, and has
verifiably drifted (dead ids, wrong bucket for `BL-163`). Regenerating it in place isn't worth doing —
there is no distinct information for it to carry once the real priority table exists; folding it into
the table above would just reintroduce the dead ids this filing exists to flag. Not fixed in the same
change as the PKT-75 merge that surfaced it, per the reviewer's request to file rather than expand that
change's scope.
...
**Superseded 2026-08-06 — the "deferred" ruling below no longer holds.** The dedicated
test-automation spec this item deferred to has been written and implemented:
`SPEC-BL-466.md` (branch `feat/bl466-wire-guards`, worktree `.worktrees/bl466-wire-guards`,
spec commit `ae185fd5`). It rules on runner form (hand-rolled `tools/run-guards.mjs`, not a
vitest port — a vitest port would put the guards under the `test` target's `dependsOn:
["^build"]`, reintroducing the BL-456 shared-checkout build hazard this item exists to keep
out) and every wiring point (`.husky/pre-commit` for Tier 1 filtered, `project.json`
`guards-tier1`/`guards-tier2` nx targets, `.github/workflows/ci.yml` full-unfiltered Tier 1 +
Tier 2 steps). See the spec's §4 for all nine ruled decisions and §5 for the acceptance
criteria (each names a BL-id, each watched red→green). The original owner-ruling text below is
retained struck through for the audit trail, not as current guidance.
...
### BL-467 — `check-plan-packets.mjs` reports 29 violations and is wired into nothing, so the plan's own structural guard has never gated anything — **Open (MEDIUM)** (2026-08-05)

**Driver.** `node tools/check-plan-packets.mjs` exits with **29 violations** against HEAD. `/usr/bin/grep -c check-plan-packets .husky/pre-commit` returns **0**. The guard exists, disagrees with the repository, and nothing tells anyone.

**What it is flagging** (measured 2026-08-05):
- Wave K packets **PKT-79..PKT-85** are missing `**budget:**` / `**orientation:**` fields.
- Four `**requires:**` fields carry prose instead of bare packet ids — a parser reads those as phantom dependency edges.
- **PKT-30 declares no `**Produces:**` while PKT-80 depends on it** — the tool's own words: *"a dependent cannot be written or verified against an undefined output."* That one is load-bearing rather than cosmetic, because PKT-80 (BL-450) is the packet that stabilises the calibration PKT-30 installs, and the two were explicitly recommended as a single unit.

**Why this is the session's recurring shape, not just a lint gap.** A check whose absence is indistinguishable from a pass. The same family as BL-435 (hand-written blocks no marker covered), BL-464 (duplicate status stamps the stamper never rewrote), BL-449 (a backup certified `ok` because the probe never ran) and BL-394 (`memory_ping` advertising guards that could not fire). In every case the artifact looked healthy precisely because the thing that would have objected was not running.

**Do not fix the 29 entries as part of wiring the guard.** They belong to the Wave K/J authoring architects, several encode deliberate decisions, and a bulk rewrite by a third party would erase intent. Wire the guard, then let the owning packets converge — or grandfather the existing set explicitly and gate only new packets, which is a decision, not a default.

**Fix candidates, none chosen:** (a) add it to `.husky/pre-commit` beside `check-backlog-markers` and `plan-status --check`, and fix the 29; (b) wire it in report-only mode first so the count is visible without blocking, then drive it to zero; (c) grandfather the current violations behind an allowlist and gate only newly-added packets.

**Related:** BL-435, BL-464 (the same "unguarded document" family, both fixed 2026-08-05 in PKT-77), BL-450/PKT-80 and PKT-30 (the dependency the `Produces:` violation actually affects).

Citations: [wip/turso-live-metrics, impl-pkt77 + claude, claude, PKT-77 close-out, 1: `node tools/check-plan-packets.mjs` → 29 violations at HEAD, 2026-08-05, 2: `/usr/bin/grep -c check-plan-packets .husky/pre-commit` → 0, 3: the tool's own PKT-30/PKT-80 `Produces:` diagnostic quoted above]
---

### BL-468 — `test-bl313`'s negative control no longer reproduces: the drizzle migrations asset still ships in every bundle but no source in the repo reads it — **Open (HIGH)** (2026-08-05)

**Driver.** With `test-bl313-graph-store-migrations-asset.mjs`'s two mechanical drifts repaired (see
CHANGELOG for BL-466's close-out), arms [1] and [2] are green and arm [3] — the negative control that
exists precisely to prove the test is not vacuous — **fails**: renaming the committed
`drizzle/migrations` asset dir away no longer reproduces the original failure.[1] `memory_stats` and
`memory_list_entities` both succeed against a bundle with the asset removed.

**This is the guard working, not the guard broken.** It is reporting that the invariant it pins has
stopped being load-bearing. A repo-wide search for the runtime consumers finds **none**:
`/usr/bin/grep -rn "_journal\|migrationsFolder" --include=*.ts --include=*.mts libs/ extensions/ apps/`
returns zero matches outside `node_modules`, and neither `graph-store/package.json` nor its
`project.json` mentions drizzle any more.[2] Yet arm [1] still passes — `bundle-extension.cjs`'s
`discoverAssets` continues to auto-discover the folder from the esbuild metafile and copy it as a
sibling of every `dist/`, so the asset is still shipped into each bundle as apparently dead weight.

**Why it matters and why it is not self-serve.** BL-313 was a CRITICAL live bug — the same table
rebuild that silently cascade-deleted all 40,930 edges — and this is its only end-to-end guard. Two
readings fit the evidence and they have opposite fixes: either (a) the drizzle migration mechanism was
genuinely superseded by the store-adapter/Turso work, the asset is vestigial, and the guard should be
repointed at whatever now carries the invariant (with the dead asset dropped from the bundles), or
(b) something outside `src/` still depends on it and the negative control merely stopped being able to
defeat a newer fallback — in which case deleting the asset would re-arm the original defect. Retiring
or relaxing a CRITICAL bug's only end-to-end guard is a contract decision; **it needs an owner ruling,
and the assertion is deliberately left RED in the meantime** rather than weakened into green.

**Do not "fix" this by deleting or loosening arm [3].** A vacuity control that is edited until it
passes is the BL-167 pattern (a test that skips exactly the case where the invariant breaks) applied
to the one guard standing over a 40,930-edge data-loss bug.

**Related:** BL-313 (the original CRITICAL bug), BL-466 (why this went seven days unnoticed),
BL-225 (a marker is not evidence), BL-167 (a guard edited into green).

Citations: [wip/turso-live-metrics, test-automator, claude, BL-466, 1: `node tools/test-bl313-graph-store-migrations-asset.mjs` → `BL-313 regression: 1 FAILED`, arms [1]/[2] all PASS, measured 2026-08-05, 2: repo-wide grep for `_journal`/`migrationsFolder` across `libs/ extensions/ apps/` → zero matches 2026-08-05, 3: `tools/test-bl313-graph-store-migrations-asset.mjs`:174-192 (the negative control), 4: `libs/data/store/store-adapter/dist/adapter-meta.js`:13]

---

### BL-470 — the shim's `ensureBackend` spawn lock coordinates shims against each other but not against launchd, so a restart can leave two backends and a self-healed pid rotation nobody chose — **Open (MEDIUM)** (2026-08-05)

**This answers BL-393's open question.** `docs/reporting/memory/STATE.md` records the 2026-08-02 incident as: doctor-tick's singleton self-heal SIGTERM'd a duplicate backend (pid 56514) 47 seconds before a rotation, and *"what spawned the duplicate has not been identified."* It is the per-session shim.

**Mechanism, from source.** Every Claude session with `memory-server` in its `.mcp.json` spawns a `soxe serve memory-server` shim. When a shim finds no live backend on the store's UDS it calls `ensureBackend` (`libs/service-proxy/src/ensure-backend.ts`), which:
- probes the socket first and no-ops if a backend is already serving (`:27`);
- otherwise takes an **O_EXCL spawn lock keyed by the singleton-key digest**, so one shim spawns and the losers wait — *"serializes the spawn across many"* (`:28-32`);
- spawns the backend **`detached: true` + `unref()`** so it survives the shim that created it (`:35`).

That lock coordinates **shim against shim**. Nothing in it coordinates against the **supervised launchd path**. So during a `soxe service restart` — the reaper SIGTERMs the running backend, launchd brings up a replacement — a concurrent shim's probe legitimately fails, it wins the lock, and it spawns a second, detached backend. Two backends now serve one store; the cross-scope singleton self-heal SIGTERMs one; a pid rotation appears with no operator action and no entry in the audit log.

**Observed twice on 2026-08-05**, both after a deliberate `soxe service restart`, both landing on the *correct* artifact — which is why it reads as benign and stayed unexplained all session. Correct artifact is not the same as intended process.

**Aggravated by agent fan-out.** Three `soxe serve memory-server` shims were live simultaneously while this was measured (pids 40788, 83087, 84637), against a single launchd-managed backend. The more concurrent sessions, the higher the probability one probes inside a restart window. A day of heavy dispatch makes a rare race routine.

**Why it matters beyond tidiness.** ADR-0007's invariant is *"at most one process may hold a write connection to a given store, and that process is the one activated on the store's socket."* This does not violate it — the self-heal restores it — but it means the invariant is maintained by **reaction** rather than by construction, and the reaction is invisible: nobody chose the rotation, nothing records why it happened, and BL-372's lesson is that a pid rotation which looks routine is exactly how an unintended artifact reaches production. On 2026-08-02 that same shape ran production on a bundle built from uncommitted source for roughly two hours.

**Fix candidates, none chosen:** (a) have `ensureBackend` consult os-unit state and decline to spawn for a store whose unit is loaded — the fallback path exists for CI and fresh installs, not for supervised stores; (b) have the restart path hold the same O_EXCL spawn lock for the duration of the teardown-and-respawn, so a shim waits rather than racing; (c) leave the race and make the self-heal loud — log the spawn provenance of the process it kills, so the rotation is at least attributable.

**Related:** BL-393 (the incident this explains), BL-372 (a rotation that looks routine is how an unintended artifact ships), BL-137 (the `ensureBackend` hardening that added probe-before-spawn), ADR-0007 §D3 (the fallback's stated purpose: CI and fresh installs).

Citations: [wip/turso-live-metrics, claude, claude, session close-out, 1: libs/service-proxy/src/ensure-backend.ts:5-35 (probe-then-O_EXCL-lock-then-detached-spawn, and the lock's stated scope: "serializes the spawn across many"), 2: libs/service-proxy/src/ensure-backend.spec.ts:5-7 (its own spec frames the concurrency case as "two sessions' shims", with no launchd arm), 3: `ps` 2026-08-05 showing three concurrent `soxe serve memory-server` shims against one launchd backend, 4: docs/reporting/memory/STATE.md — BL-393's "what spawned the duplicate has not been identified", 5: two observed rotations on 2026-08-05, both following a deliberate restart, both onto the correct artifact]
---


### BL-474 — `clustering-e2e.test.ts` step 3 races the always-on drain chain for the shared `_bgSlot` mutex and can hang the full 30s test bound — **Open (MEDIUM)** (2026-08-06)

**Symptom.** `clustering-e2e.test.ts` step 3 (`runEnrichPassOnDb`, turso backend) timed out at exactly 30008ms in one isolated `npx nx test memory-server -- --run clustering-e2e` run (17/18 passed, only that test failed). Two immediately-following isolated runs of the identical command passed clean, turso step 3 completing in 100ms and 158ms.

**NOT caused by the concurrent BL-472 merge** — investigated as a suspected regression from BL-472 (bounded shutdown drain, `223e583a`) and exonerated by code-path analysis: BL-472's `runDrainPassGuarded()` refactor is a behavior-preserving wrap (identical `_drainInFlight`/`withBackgroundSlot('drain', ...)` body, same "never throws" contract) and its only new surface — `waitForDrainSettled()`/`_drainInFlightPromise` — is consumed exclusively by `coordinatedShutdown` step 0 in `backend.ts:269-298`, a path `clustering-e2e.test.ts` never reaches (it calls `runEnrichPassOnDb` directly, never shutdown).

**Actual mechanism (pre-existing, BL-382/BL-346 architecture).** `runEnrichPassOnDb`'s heal step acquires `withBackgroundSlot('enrich-heal', ...)` (`index.ts:2266-2268`), a serial FIFO mutex (`_bgSlot`, `index.ts:2563-2583`) shared with the background drain chain's `withBackgroundSlot('drain', () => runDrainPass())` (`index.ts:2709`). `scheduleNextDrain()` runs **unconditionally at module import** (`index.ts:2903`), arming a real drain pass at `DEFAULT_DRAIN_FLOOR_MS` (30s, `index.ts:2026`) with no priority or preemption against the enrich-heal caller. Confirmed live: added temporary `backgroundSlotHolder()` polling instrumentation (test seam, already exported) to `clustering-e2e.test.ts`, reverted after — `git status` clean — and observed the drain chain hold `_bgSlot` continuously for **~12.5s** in one run. Whether a given `enrich-heal` call queues behind that window before its own 30s test bound expires is a race, explaining pass/fail non-determinism under load (measured load avg 67/132/97 on 10 cores during the failing run — two other agents mid-`nx build`).

**Why this is fileable regardless of trigger.** `runEnrichPassOnDb` is the steady-state path every `memory_write` enrichment tick reaches in production (test's own docstring, `index.ts:2024`). If the always-on drain chain can hold the shared mutex for 12s+ under real contention with no preemption, every write-triggered enrich tick pays that latency too — not just the test. A 30s test bound is marginal against a documented 12s+ contention window; either the mutex needs a priority/preemption model, or the test needs env-level isolation from the background drain chain (e.g. `SOX_EMBED_DRAIN_FLOOR_MS` set far outside the test's own runtime), or both.

**Not fixed here** — scope was diagnosis only (dispatched as a debug investigation). No worktree counterfactual against pre-BL-472 `893b42c4` was completed (would require a fresh `pnpm install`, judged unnecessary given the code-path exoneration above is already conclusive: BL-472's new code is provably unreachable from this test).

**Related:** BL-382 (drain chain requirement 1 — debounced coalescing), BL-346 (background-slot anti-stampede origin), BL-348 (narrowed the slot's clustering scope, established the `_bgSlot` contract this item's contention lives inside).

Citations: [main @ ac5ec29f, debugger, claude, clustering-e2e-timeout-investigation, 1: extensions/bundles/sox-memory-bundle/members/memory-server/src/index.ts:2221-2389 (runEnrichPassOnDb), 2: extensions/bundles/sox-memory-bundle/members/memory-server/src/index.ts:2563-2583 (withBackgroundSlot mutex), 3: extensions/bundles/sox-memory-bundle/members/memory-server/src/index.ts:2698-2744 (runDrainPassGuarded, BL-472 refactor), 4: extensions/bundles/sox-memory-bundle/members/memory-server/src/index.ts:2762-2772,2903 (scheduleNextDrain armed unconditionally at import), 5: extensions/bundles/sox-memory-bundle/members/memory-server/src/backend.ts:269-298 (coordinatedShutdown step 0, sole consumer of waitForDrainSettled), 6: extensions/bundles/sox-memory-bundle/members/memory-server/clustering-e2e.test.ts:331-333, 7: git show 223e583a, 8: /tmp/gate-postmerge.log (original 30008ms failure), 9: /tmp/dbg-isolated-run.log (reproduced 30008ms timeout, load avg 67/132/97, tree confirmed clean via tools/check-suite-tree-state.mjs), 10: /tmp/dbg-instrumented-run.log lines 156-184 (bgSlotHolder=drain sustained ~12.5s)]

---

### BL-475 — allocating a BL-id from a worktree leaves the MAIN checkout dirty and failing its own marker check, with no cleanup path — **Open (MEDIUM)** (2026-08-06)

**What's wrong.** `tools/allocate-bl-id.mjs` correctly resolves the shared registry root via
`git rev-parse --git-common-dir` + `..` (BL-416's fix, per the owner's shared-registry ruling), so an
agent working in `.worktrees/<slug>` writes its `RESERVED` placeholder heading into the **main
checkout's** `BACKLOG.md`. That is the intended cross-worktree design and it is what makes the id
allocation race-free. The defect is the *side effect*: the main checkout is left with an uncommitted
modification the allocating agent cannot commit (it is not on that branch and must not commit there),
and `check-backlog-markers.mjs` immediately goes red against it because the stub adds a heading the
header count does not know about.

Observed live 2026-08-06:

```
$ cd /Users/nix/dev/ai/sox-ecosystem && git status --porcelain BACKLOG.md
 M BACKLOG.md
$ node tools/check-backlog-markers.mjs
  FAIL  header claims 87 open; markers derive 88. Regenerate the header.
check-backlog-markers: 1 violation(s).  (exit 1)
```

**Why it matters beyond tidiness.** The main checkout is shared by every concurrent agent. A red
`check-backlog-markers` there blocks the **pre-commit hook for everyone**, not just the allocator —
the next agent to commit anything inherits a failing gate it did not cause and has no context for.
It also blocks a `git merge` into `main` outright (`Your local changes to the following files would
be overwritten by merge: BACKLOG.md`), which is how this surfaced: a merge of `feat/pkt75-backlog-tooling`
aborted on a stub left by an allocation from that same worktree. Worse, the stub is indistinguishable
from a genuine in-progress filing, so the safe-looking remedy (`git restore BACKLOG.md`) silently
destroys a real reservation if the allocating agent has not yet written its content.

**Not the same as BL-416** (resolved 2026-08-06), which was that the scripts resolved the *wrong*
root (`--show-toplevel`, per-worktree) and so forked the registry. This is the unhandled consequence
of the *correct* shared root.

**Fix candidates, none chosen.** (a) Have the allocator regenerate the header count when it writes the
stub, so the main checkout stays green — smallest change, but leaves the file dirty. (b) Keep the
reservation out of `BACKLOG.md` entirely — a sidecar registry file (e.g. `.bl-id.reserved`) that
`check-backlog-markers.mjs` reads but that is not itself a heading, so allocation never dirties the
tracked file. (c) Make the allocator's write transactional with the caller's commit — reserve in the
lock dir only, and materialize the heading in the worktree that is actually filing. **(b) or (c)
looks right; (a) treats the symptom and still leaves every concurrent agent with a dirty shared file.**

**Acceptance, naming BL-475.** Allocate an id from a worktree, then assert in the **main** checkout
that (1) `git status --porcelain BACKLOG.md` is empty and (2) `node tools/check-backlog-markers.mjs`
exits 0 — both fail today, which is the red arm. Additionally assert a `git merge` into `main`
succeeds immediately after an allocation from a worktree, since that is the failure that surfaced this.
...
**Framing note (2026-08-06, per `docs/decisions/0009-backlog-source-of-truth.md`):** this defect is real and stays open regardless of which store is authoritative — it mints ids colliding with the authoritative markdown, not "blocks phase-3 here." `BACKLOG.md` is the source of truth for `BL-*` in this repo (ADR-0009); the graph is a read-only mirror. The collision risk (a graph auto-allocation landing on an id `BACKLOG.md`/`CHANGELOG.md` already used) is exactly as real under that ruling as it would be under phase-3 — the fix recommendation below is unaffected.

**Cross-repo dependency defect: the defect lives upstream, the damage lands here.** `computeNextHumanId` (`~/dev/sdlc-experiments/arm-rf/rf-run/entrypoint/backlog/src/store/ids.ts:25-55`, function backing `backlog_create_item`'s auto-allocation when called without `idOverride`; package `@adhd/backlog`, git remote `github.com:PseudoSky/adhd.git`) derives the next `humanId` as:

```js
const existing = store.graph.queryNodes({ kind: 'generic', tags: [BACKLOG_ITEM_TAG], namespace: repo, metadata: { family } });
let max = 0;
for (const node of existing) { const match = /-(\d+)$/.exec(meta?.humanId ?? ''); if (match) max = Math.max(max, Number(match[1])); }
return `${family}-${String(max + 1).padStart(3, '0')}`;
```

There is no stored counter anywhere in `store/*.ts` (grepped for `counter|sequence|next_id|nextId`, zero hits) — this is the entire mechanism, a pure `MAX(live graph nodes' humanId for this family) + 1`, recomputed on every call. `queryNodes` (traced into `@adhd/sox-graph-store`, `libs/data/graph/graph-store/src/index.ts:1035`, hardcodes `liveOnly:true` into `buildNodeFilterClause`, i.e. `t_invalid IS NULL`) only ever sees nodes that exist **in the graph** — it has no visibility into `BACKLOG.md` or `CHANGELOG.md` history that was never imported into the graph.

**Observed live 2026-08-06.** `backlog_create_item` (no `idOverride`) issued `humanId: BL-437` for a brand-new mutex-contention finding. `BL-437` already existed as resolved history — `CHANGELOG.md:272` (then-current line), a same-day-resolved, unrelated hybrid-search topic-boost fix — but had never been imported into the graph, so the allocator's `MAX()` scan never saw it and treated the id as free. The collision was caught only because a human-readable item happened to land on the same id as recently-resolved content; a less recognizable collision would have silently overwritten provenance. (Recovered separately: the two `BL-437` nodes were merged into the surviving `BL-474`, by `plan-orchestrator`, not filed here as a residual defect — see `DEBT-BACKLOG-TOOL-MARKDOWN-SPLIT-BRAIN-001`'s notes for that correction.)

**Why this is a `sox-ecosystem` item and not an `@adhd/backlog` item.** Per the repo owner's explicit direction: the defect is upstream, but a defect filed in a repo nobody on this task is watching will not be actioned, and this repo is the one carrying the risk — the next agent to file through the graph with no `idOverride` can collide again. This item does not authorize editing `@adhd/backlog`; it exists so the risk is tracked where it is felt.

**Confirmed NOT currently live, but only by luck of timing.** As of this filing, the graph's live high-water-mark for family `BL` is `BL-475` (backfilled during the 2026-08-06 markdown↔graph reconciliation, `DEBT-BACKLOG-TOOL-MARKDOWN-SPLIT-BRAIN-001`), which equals the highest `BL-<n>` referenced anywhere in `BACKLOG.md`+`CHANGELOG.md` (also `475`, confirmed by a loose grep for `BL-[0-9]+` across both files, not just headings). So `computeNextHumanId` will correctly issue `BL-477` next. **This is coincidental self-healing from a one-time backfill, not a fix** — the moment any markdown-only id above the graph's live max is filed (exactly how `BL-437`/`BL-472`/`BL-475` each arose), the gap reopens.

**Compounding factor, found while investigating this item: `CHANGELOG.md` uses TWO heading grammars for resolved items** — `### BL-<n> (...) — <title>` and the current/dominant `## [Unreleased] — BL-<n>: <title>` (322 distinct ids found under the latter alone, spanning `BL-1`..`BL-472`, vs 114 then in the graph — 300 missing). Any process (human or tool) trying to keep the graph's high-water-mark current by scanning only `### BL-<n>` headings will systematically undercount and reopen this gap. See `DEBT-BACKLOG-TOOL-MARKDOWN-SPLIT-BRAIN-001`'s notes for the full sweep.

**Fix sketch (recommendation only — this item does not authorize implementing it, in this repo or upstream).** Two candidates, in preference order:
1. **Seed-and-never-undercut** — at the point the graph is made authoritative for a repo (phase-3 cutover), one-time-import the true historical high-water-mark for every family from markdown into the graph (exactly what this session's backfill did, manually) and rely on `computeNextHumanId`'s existing live-`MAX()` logic from then on. Cheap, no upstream code change, but fragile to exactly this session's failure mode if any markdown-only id is ever filed later without going through the graph first (impossible under a genuinely-enforced phase-3, but phase-3 is not yet mechanically enforced — see `DEBT-BACKLOG-RENDER-MARKDOWN-SHAPE-MISMATCH-001`).
2. **Scan markdown at allocation time** — have `computeNextHumanId` additionally read the repo's `BACKLOG.md`/`CHANGELOG.md` (both heading grammars) and take the max across graph + markdown. More robust, but couples a generic graph-store-backed package to a markdown-file convention that varies per consumer repo, and adds file I/O + parsing cost to every single-item allocation.

Recommend (1) as the immediate/cheap mitigation (already informally true here) with an explicit, repeatable "seed on cutover" step documented for phase-3 migrations generally, and (2) considered only if repos other than `sox-ecosystem` hit the same failure mode.

**Acceptance, naming BL-476.** Not fixable in this repo (upstream package). Acceptance is: either `@adhd/backlog` ships a fix (either candidate above) and this item is verified against it, or this repo formally documents and re-runs the "seed on cutover" step as a standing runbook entry rather than a one-time manual backfill, closing this item as mitigated-by-process rather than fixed-in-code.

## Related, lower-severity: `backlog_link_related` returns `null` on both success and failure — indistinguishable without re-querying

**Found during the same investigation.** `backlog_link_related({humanIdA:"BL-472", humanIdB:"BL-474"})` returned `{"result":null}`. Independently queried `backlog_dependency_graph` and confirmed the edge landed: `{"from":"BL-472","to":"BL-474","rel":"RELATES_TO"}`. So `null` is this API's normal success return (void), not evidence of failure — **but the API contract makes success and failure identical at the call site.** A caller cannot distinguish "the link was created" from "the call silently no-op'd" without a second, independent read — which is exactly what happened here: a `null` return was flagged (correctly, as due diligence) as a suspected safety issue before being confirmed benign by cross-checking the dependency graph. Recommend (not authorized to implement, upstream): `backlog_link_related` return `{ok:true}` or the created/existing edge itself, not `null`, so success is self-evident rather than requiring a second call to prove.

**Severity:** LOW for this sub-finding on its own (verified non-destructive this round) — filed alongside the id-allocator defect rather than as a fully separate item since both surfaced from the same investigation and share the same upstream package.

Citations: [wip/turso-live-metrics, devops-engineer, claude, backlog-markdown-migration follow-up, 1: ~/dev/sdlc-experiments/arm-rf/rf-run/entrypoint/backlog/src/store/ids.ts:25-55 (computeNextHumanId), 2: libs/data/graph/graph-store/src/index.ts:1035,646 (queryNodes liveOnly hardcode), 3: live repro — backlog_create_item with no idOverride issued BL-437, colliding with CHANGELOG.md's already-resolved hybrid-search topic-boost fix, 2026-08-06, 4: DEBT-BACKLOG-TOOL-MARKDOWN-SPLIT-BRAIN-001 (the reconciliation this was found during, and its notes for the two-heading-grammar finding), 5: backlog_link_related({humanIdA:"BL-472",humanIdB:"BL-474"}) → {"result":null}; backlog_dependency_graph confirmed the edge exists, 2026-08-06]

---

### BL-478 — `backlog_render_to_markdown`'s heading grammar and `check-backlog-markers.mjs`'s heading grammar are incompatible — settled by ADR-0009, not a blocker to fix — **Open (LOW, sweep hazard)** (2026-08-06)

**Re-framed 2026-08-06 — this is NOT a regeneration blocker.** It was originally filed as "phase-3 cannot regenerate `BACKLOG.md` until these are reconciled," which presumes regeneration is the goal. It is not: `docs/decisions/0009-backlog-source-of-truth.md` (ACCEPTED, owner pseudosky, 2026-08-04) already rules that `BACKLOG.md` is the source of truth for `BL-*` in this repo, that the graph is a **read-only mirror**, and that regeneration is explicitly rejected — its own §"Why regeneration is not viable" quotes this exact `renderItemBlock` mismatch as its evidence, independently, two days before this item was filed. Neither the renderer nor `check-backlog-markers.mjs` needs to change. **Do not implement a dual-grammar checker or a renderer fix against this item** — both would build the regeneration path ADR-0009 rejected.

**What the finding is still worth for:** `@adhd/backlog`'s renderer (`entrypoint/backlog/src/markdown.ts:261-274` in the separate `PseudoSky/adhd` repo) emits `### {humanId} — {title}` then a **separate** `**Status:** {STATUS}` body line, matching its own parser `detectStatus` exactly — internally consistent, just a different grammar than `check-backlog-markers.mjs`'s inline-heading-marker convention. Both are individually correct; this item exists so a future sweep doesn't assume `###`-only parsing covers all resolved history. That's the same lesson as the two `CHANGELOG.md` heading grammars found during this session's tiered backfill (`### BL-<n>` vs `## [Unreleased] — BL-<n>:`) — **any tool that keys on one heading shape will silently undercount the other**, and this item is the equivalent finding on the graph-render side rather than the CHANGELOG side.

**Acceptance, naming BL-478.** No code fix owed by this item. Closes when noted as informational/documented, or superseded if ADR-0009 is ever revisited (owner-only decision, not to be re-opened by an agent).

**Related:** ADR-0009 (`docs/decisions/0009-backlog-source-of-truth.md`, the ruling that resolves this), `DEBT-BACKLOG-RENDER-MARKDOWN-SHAPE-MISMATCH-001` (the graph-only record, same re-framing applied there), `BL-476` (the allocator defect — real regardless of which store is authoritative, since it mints ids colliding with the authoritative markdown).
...
### BL-479 — `tools/test-bl*.mjs` guards that spawn `git` inside a scratch fixture repo inherit the invoking shell's `GIT_INDEX_FILE`/`GIT_DIR`, so running one while those env vars point at a real checkout silently corrupts that checkout's index — **Open (HIGH)** (2026-08-06)

**Driver.** While verifying BL-466's pre-commit wiring (AC-c), this worktree's git index turned up
several stage-0 entries pointing at blob SHAs `git cat-file -e` cannot resolve — `BACKLOG.md`,
`CHANGELOG.md`, a phantom top-level `STATE.md` (no such path exists in `HEAD` or on disk; the real
tracked file is `docs/reporting/memory/STATE.md`), and two files, `fileA.txt`/`fileB.txt`, that do
not exist anywhere in this repo's tree at all.[1] Those two names are not a coincidence:
`tools/test-bl409-pathspec-commit.mjs` creates exactly `fileA.txt`/`fileB.txt` inside its own
`fs.mkdtempSync` scratch repo to exercise pathspec-commit semantics.[2] Their appearance as real
entries in **this worktree's real index** — not the scratch repo's — is the fingerprint of the
actual defect.

**Root cause, confirmed.** `test-bl409-pathspec-commit.mjs`'s `sh()` helper calls
`execFileSync(cmd, args, { cwd, encoding: 'utf8' })` with no `env` override.[3] `git` resolves the
repository via `GIT_DIR`/`GIT_INDEX_FILE`/`GIT_WORK_TREE` environment variables **before** falling
back to `cwd`-based discovery. If those variables are set anywhere in the calling process tree —
this session set `GIT_INDEX_FILE=.git/worktrees/bl466-wire-guards/index` when invoking
`.husky/pre-commit` directly to reproduce what a real `git commit` would run (§ AC-c verification,
since git worktrees do not consult a per-worktree hooks directory — see the related finding
below) — every `git` call the guard makes with only `cwd` set, never an explicit `env`, silently
operates on the **real, invoking checkout's index** instead of its own scratch fixture, because the
inherited env var wins. The guard's fixture commits (`fileA.txt`/`fileB.txt`, plus whatever
`BACKLOG.md`/`CHANGELOG.md`/the phantom `STATE.md` entries came from a different guard's fixture in
the same family — `test-bl435`, `test-bl464`, `test-bl454` and others all touch `BACKLOG.md`/
`plan-status.mjs` output by name) landed in the shared index instead of the scratch repo, and some
of those never-committed blobs were later reclaimed: `git gc`/`git gc --prune=now` does not treat an
index-only blob as a protected root the way a ref or reflog entry is, so once a `gc` runs anywhere
that shares this repo's object store, the entry survives in the index but its blob is gone —
verified: `find .git/objects -name <sha>` (main checkout, the shared object store every worktree
reads via `--git-common-dir`) returns nothing, and a loop over every `.git/objects/pack/*.idx` with
`git verify-pack -v` matching the same SHA also returns nothing, for all affected SHAs.[4]

**All 17 guards under `tools/test-bl*.mjs` share this exposure.** Every one of them that spawns
`git` against a scratch/fixture directory does so via `execFileSync`/`spawnSync` with only `cwd` set
— none pass an explicit `env` that strips `GIT_DIR`/`GIT_INDEX_FILE`/`GIT_WORK_TREE`. The exposure
is latent (it requires an ambient env var most invocations never set), but it is not narrow: `sh
.husky/pre-commit` with `GIT_INDEX_FILE` exported is exactly the technique this session used to
verify BL-466's AC-c because git worktrees do not have their own hooks directory (git resolves
`core.hooksPath`/the default `.git/hooks/pre-commit` from the **shared main checkout**, confirmed
via `GIT_TRACE=1 git commit` — `tools/install-git-hooks.mjs` installing into
`.git/worktrees/<name>/hooks/` has no effect on what `git commit` actually invokes from a linked
worktree). Any future agent using the same verification technique — or any CI/tooling context that
legitimately sets `GIT_INDEX_FILE` for its own reasons and then shells out to one of these guards —
reproduces this corruption.

**Not this session's index-hygiene failure.** This agent's only pathspec-less `git commit` calls
(two, both undone via `git reset --soft` back to `ae185fd5`, never `git reset --hard`, never `git
stash`) targeted a single file (`tools/verify-native-abi.mjs`) and never staged `BACKLOG.md`,
`STATE.md`, `fileA.txt`, or `fileB.txt` — those entries were already present before this session's
first `git status` call, evidence the corruption predates this session and most plausibly traces to
an **earlier** session in this same worktree that ran the guard suite (`SPEC-BL-466.md` §2 records
"actually running in the worktree" for 13 of the 17 guards, including `bl409`) under an ambient
`GIT_INDEX_FILE`.

**Why a pathspec-scoped commit is the mitigation, not just the house rule.** `git commit-tree`/
`write-tree` **do** validate that every referenced blob exists when constructing the new tree (`git
commit BACKLOG.md CHANGELOG.md -m ...` failed loudly — `error: invalid object ... for
'CHANGELOG.md'` — before this session repaired the affected entries), so the corruption is
self-announcing on the next pathspec commit that happens to touch an affected path, and blocks *any*
commit that omits the pathspec markers this repo already mandates. It is silent only for a
pathspec-less `git commit`, which is banned outright by this repo's own house rules for the
unrelated reason (BL-409) — so this defect is close to fully mitigated by rules this repo already
enforces, but the enforcement is procedural, not structural: nothing stops a guard from polluting
the index in the first place.

**Repair performed this session (non-destructive, scoped).** For each of the 5 affected paths,
confirmed either (a) the working-tree content is byte-identical to `HEAD`'s committed version
(`BACKLOG.md`/`CHANGELOG.md` — safe to `git add` back to a known-good state with zero data loss,
since the corrupted blob was unrecoverable regardless) or (b) the path corresponds to no real file
in `HEAD` or on disk at all (`STATE.md` top-level phantom, `fileA.txt`, `fileB.txt` — `git rm
--cached` to remove the phantom entry, restoring index/HEAD agreement). No other paths were touched;
`node -e` script enumerating `git ls-files --stage` and probing each blob with `git cat-file -e`
found exactly these 5 broken entries, zero more, after each repair step.[5]

**Acceptance, naming BL-479.** Every `tools/test-bl*.mjs` guard (and `tools/run-guards.mjs`, and any
future guard) that spawns `git` against a scratch/fixture repo passes an explicit `env` stripping
`GIT_DIR`/`GIT_INDEX_FILE`/`GIT_WORK_TREE`/`GIT_COMMON_DIR` (or sets `GIT_DIR=<scratchdir>/.git`
explicitly) so it can never resolve to the invoking checkout regardless of ambient environment. RED:
export `GIT_INDEX_FILE=<real repo's index>` in the shell, run any affected guard, observe the real
index gains the guard's fixture entries (`git ls-files --stage | grep fileA.txt` before the fix).
GREEN: same repro, guard's fixture entries never appear in the real index.

**Related:** BL-463 (staged entries outliving their agent — same shared-index hazard family, but
assumes staged content is stale-but-valid; this item is the case where the mechanism actively
injects foreign entries and the blob becomes unrecoverable), BL-457/BL-465 (`commit-mine.mjs`'s
private `GIT_INDEX_FILE` seeded fresh from `HEAD` is unaffected by this defect by construction —
recommended for any BACKLOG.md/CHANGELOG.md edit while this is open), BL-475 (a related but distinct
worktree/shared-registry hazard: `check-backlog-markers.mjs`/`check-bl-id-integrity.mjs` validate
the **main checkout's** `BACKLOG.md` even when invoked from a worktree, so an unrelated drift in
main blocks every worktree's pre-commit hook — hit live while committing this very entry, worked
around per BL-475's own documented path (`tools/commit-mine.mjs`, which explicitly bypasses hooks)).

Citations: [feat/bl466-wire-guards, test-automator, claude, BL-466 SPEC AC-c verification, 1: `node -e` script enumerating `git ls-files --stage` + `git cat-file -e` per blob, this worktree, 2026-08-06 → `["BACKLOG.md","CHANGELOG.md","STATE.md","fileA.txt","fileB.txt"]` before repair, 2: tools/test-bl409-pathspec-commit.mjs:1-40 (`fileA.txt`/`fileB.txt` scratch-repo fixture names), 3: tools/test-bl409-pathspec-commit.mjs:35-37 (`sh()` — `execFileSync(cmd, args, { cwd, encoding: 'utf8' })`, no `env`), 4: `find .git/objects -name <sha>` + `git verify-pack -v` over every `.git/objects/pack/*.idx`, main checkout, 2026-08-06, empty for all 5 affected SHAs, 5: repair verified via re-run of the same `node -e` enumeration script after each `git add`/`git rm --cached`, converged to `[]`, 6: `GIT_TRACE=1 git commit --allow-empty` in `.worktrees/bl466-wire-guards` → `run_command: .../.git/hooks/pre-commit` (main checkout path, not `.git/worktrees/bl466-wire-guards/hooks/pre-commit`), 2026-08-06]

---

### BL-480 — `test-bl266-bundle-invariants.mjs` arm (d), run for real for the first time under BL-466's Tier 2 wiring, confirms `bundle-extension.cjs` builds succeed on a real type error — expected given BL-248, but now a live, watched assertion instead of an always-skipped one — **Open (LOW)** (2026-08-06)

**Driver.** BL-466's Tier 2 wiring is the first time `tools/test-bl266-bundle-invariants.mjs` has
ever been invoked with `--build-cmd`/`--source`/`--rebuild-cmd` supplied for real (BL-469 fixed the
guard's own honesty about arms (c)/(d)/(e) being skipped without those flags; nothing before this
item ever supplied them). With the real memory-server `bundle-extension.cjs` invocation wired in
(`tools/run-guards.mjs`'s `buildBl266Args`, derived live from
`extensions/bundles/sox-memory-bundle/members/memory-server/project.json`'s own build command, not
hardcoded), arm (d) — "typecheck as a first-class gate" — runs for real and reports: `build command
SUCCEEDED despite a real type error — typecheck is not gating this build target`.[1]

**Not a regression, not a surprise given BL-248 — but previously unverified.** This repo's own
documented architecture is exactly this: `tools/bundle-extension.cjs` strips types without checking
them, and `nx typecheck` is a deliberately separate gate specifically because `build` does not imply
it (`AGENTS.md`'s "`typecheck` is not optional, and `build` does not imply it" section, and BL-248's
finding that memory-server shipped 15 real TypeScript errors through a green `build,lint,test`
sweep).[2] CI's own `ci.yml` already runs `nx affected -t typecheck` as an independent step for
exactly this reason.[3] So the *system* (build + typecheck as two gates) already covers what arm (d)
is probing for at the single-component level — this item exists because arm (d) had never actually
executed before today, so nobody had verified `bundle-extension.cjs` in isolation matches its own
documented contract until BL-466 wired real args into it.

**Acceptance, naming BL-480.** Either (a) confirm arm (d)'s assertion is testing the wrong layer —
`bundle-extension.cjs` build succeeding without typechecking is BY DESIGN, so the guard's expectation
that the *build command* should fail on a type error is itself the thing that needs correcting (not
`bundle-extension.cjs`) — and rewrite arm (d) to assert what's actually true (build succeeds, a
SEPARATE `nx typecheck` on the same target catches it), or (b) if the intent really is "no single
command path should ever be able to ship untyped code," identify the deployment path (if any) that
runs `bundle-extension.cjs` without a preceding/gating `nx typecheck` and close that gap instead.
**Do not just flip arm (d)'s assertion to expect success** without first deciding which of (a)/(b)
is correct — that would be exactly the "loosen a guard to get a green wiring pass" trap this
session's own SPEC-BL-466.md explicitly forbids (§3, "Explicitly out of bounds").

**Related:** BL-248 (the original build-vs-typecheck-are-separate-gates finding this item
re-confirms at the component level), BL-469 (made arm (d) honestly report SKIP instead of a false
PASS — the prerequisite that made this item's finding possible to even observe), BL-466 (the wiring
that supplied real `--build-cmd`/`--source`/`--rebuild-cmd` for the first time).

Citations: [feat/bl466-wire-guards, test-automator, claude, BL-466 SPEC §5 Decision 6 (bl266 driver args), 1: `node tools/test-bl266-bundle-invariants.mjs --outdir ... --build-cmd "node tools/bundle-extension.cjs ..." --source .../src/index.ts` inside an isolated worktree with memory-server's real dist built, 2026-08-06 → `[FAIL] (d) typecheck gate — build command SUCCEEDED despite a real type error`, 2: AGENTS.md "⛔ AGENT SEQUENCE — BUILD VIA NX TARGETS" section, "`typecheck` is not optional, and `build` does not imply it" + BL-248's 15-type-error finding, 3: .github/workflows/ci.yml "Typecheck (nx affected)" step, independent of "Build (nx affected)"]
