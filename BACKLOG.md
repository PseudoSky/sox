...
**Root cause is lifecycle, not commit mechanics.** `git add` is how the entries get there, but the hazard is that **stopping or completing an agent does not clean up its index entries**. They outlive the agent, silently, and the next agent to run a pathspec-less commit inherits them. `tools/commit-mine.mjs` is immune by design — it seeds a private `GIT_INDEX_FILE` from HEAD — which is why it kept reporting "N path(s) staged in the SHARED index by someone else" while the entries accumulated unnoticed.
