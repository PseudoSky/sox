...
### BL-399 — live `store.error: no such column: meta`, swallowed, on a healthy store whose schema declares that column — **Open (HIGH)** (2026-08-01)

**Observed on the live store**, on freshly-deployed artifact `5e8e1fcc8625` (pid 18521), in the durable telemetry:[1]

```
2026-08-01T19:34:52.213Z  error  store.error
  "prepare failed: Parse error: no such column: meta"
```

**It is swallowed.** The timestamp correlates with a `memory_near_duplicates({threshold: 0.5})` call that **returned 547 pairs successfully**. So a query failed at prepare, something caught it, and the tool reported a normal result. No error surfaced to the caller, nothing in `memory_ping`, and integrity still reports `overall: ok` with all five probes validated. Whatever that query was meant to contribute is silently missing from the answer — the caller cannot tell.

**The column is not missing from the declared schema**, which is what makes this worth real investigation rather than a quick patch. `libs/data/graph/graph-store/src/index.ts` declares `meta TEXT` on `edge` (:55, and :141 in the drizzle mirror) and `node` has a `meta` column read at :855 (`SELECT meta FROM node WHERE rowid = ?`). So one of the following is true and they need very different fixes:
1. the **live** store's table genuinely lacks the column (schema drift the adapter's migrator did not reconcile — which would be a direct hit on the owner's standing requirement that the adapter must verify and migrate its own store, cf. BL-347/BL-352);
2. the query targets a table or alias where `meta` does not exist (e.g. a join or CTE that drops it) and only fails on Turso's stricter parser;
3. Turso rejects the specific statement shape for an unrelated reason and reports it misleadingly as a missing column — this backend has already produced one such misleading diagnostic today (BL-385's `database disk image is malformed` on a perfectly healthy store).

**Do not assume (1).** That is the intuitive reading and it is the one that would send someone to hand-repair a live store. Establish which by capturing the actual failing SQL first — the current log line records the driver's message but not the statement, which is itself the gap that made this hard to place.

**Related, and probably the same call path:** BL-386 changed `memoryNearDuplicates` to read `e.weight` with a fallback to `e.metadata`, and `getEdges` selects `e.meta AS e_meta` (:1130). BL-398 covers the `weight` semantics of that same read. If the fallback path is what fails here, these three items share one query.

**Fix sketch:** first, make `store.error` log the statement (or a fingerprint of it) alongside the driver message — without that, every future instance of this costs the same forensic detour. Then determine which of the three cases above holds, by querying the live table's actual columns (read-only, against a **copy**, `.db` + `-wal` together per BL-330). Only then choose between a migrator fix and a query fix. Whatever is found, the swallow must go: a caught prepare-failure that still returns a confident result is the same shape as BL-381 (dead near-dup), BL-384 (entity search silently a LIKE scan) and BL-391 (federated store contributing `[]`).

**Severity:** HIGH — an unexplained SQL failure on the live production store that no health surface reports, on the store's own graph tables. Downgrade only once the scope is actually known.
...
### BL-401 — `@adhd/sox-telemetry` interface is published but BL-351's acceptance is not met: no consumer migration, no status-surface wiring, no live-spawned verification — **Open (HIGH)** (2026-08-01)
...
**What exists and is proven (real tests, not aspirational):** `libs/observability/sox-telemetry` — `initTelemetry`, `declareStages`/`withContendedStage` (wait/work pairing, closed-union stage/path), `DurableJsonlSink` (writeSync-durable, BL-365; pruner anchored on the full `<component>-<ISO-date>` shape, closing the §5.8 footgun), `telemetrySelfCheck()`. 6/6 tests green; `typecheck`, `typecheck-tests`, `lint`, `test`, `build` all pass for the project in isolation.[1]

**What is NOT done, and is exactly what BL-351's acceptance line requires ("two different packages emit spans that join on one trace-id... every emitted metric is reachable from the status surface without reading a log file... verified on a real spawned service, not in-process"):**

1. **`libs/memory-core/src/telemetry.ts` still owns its own `RotatingJsonlWriter`** — a near-duplicate of the new `DurableJsonlSink`, not migrated. Deliberately not touched this pass: `telemetry-crash-durability.spec.ts` and `telemetry.spec.ts` are load-bearing regression tests that rely on `SOX_MEMORY_LOG_*` being re-read **per call** (so a test can flip an env var mid-run without reconstructing the writer); `DurableJsonlSink` takes its configuration at construction time by design (env resolution belongs at the composition root per BL-351 §5.0's dependency shape, not inside the sink). A safe migration needs to either (a) have memory-core's composition root re-construct/reconfigure the sink on every env-affecting call site the tests exercise, or (b) add an explicit `reconfigure()`/hot-reload path to `DurableJsonlSink` and re-verify BL-365's crash-durability guarantee still holds through it. Do this as its own reviewed change, not a rushed swap that risks silently breaking BL-365.
2. **No second package consumes `@adhd/sox-telemetry` yet** — `store-adapter`, `embedding-provider`, and `graph-store` still have zero shared instrumentation, which is the actual BL-319/BL-331 payoff (stage-level attribution inside embed compute) the substrate exists to unlock.
3. **`memory_ping`/`memory_stats` (BL-334) do not call `telemetrySelfCheck()`.** The self-check exists and is unit-tested but is not reachable from the live status surface, which is the specific BL-353 failure mode ("the data is written when somebody looks... nobody looked for two days") the design calls out as insufficient on its own (§5.7).
4. **The OpenTelemetry SDK itself is not wired.** `docs/research/observability-substrate.md` §5.1/§5.6/§5.8 specifies `BasicTracerProvider` + `AsyncLocalStorageContextManager`, a `JsonlSpanProcessor` implementing `SpanProcessor.onStart`/`onEnd` against the standard's own extension point, and a pull-only `MeterReader` with an exponential-histogram view. What ships instead is the substrate's own span/metric model — same event names, same units, same durability and hang-visibility guarantees — but NOT literally an OTel `SpanExporter`/`SpanProcessor`. `@opentelemetry/api` is a declared dependency and unused by the current implementation. Fine as an intermediate step (the wire format is stable and the doc's `@opentelemetry/api`-facade-only dependency shape for consumers is unaffected either way), but the acceptance line's "join on one trace-id through the shared API" implies the real API, and this should be closed before anyone treats BL-351 as done.
5. **No verification against a real spawned service.** Everything above is proven in-process, in vitest. BL-351's acceptance explicitly requires the opposite.
6. **Metric persistence (§5.8, activity-triggered snapshot into the same durable sink) is not implemented** — `telemetrySelfCheck()` is a live in-memory view only, labelled `"window": "since process start"` so it cannot be mistaken for a durable cumulative counter, but nothing snapshots it to disk, so a crash loses everything the in-memory aggregate held.

**Fix sketch:** work through 1–6 in roughly that order — (2) and (3) are the highest-leverage next steps (they make the substrate *used*, which is the whole point per §5.7), (1) is the highest-risk (touches load-bearing tests), (4) and (6) are the parts of the original design most defensibly deferrable, and (5) is the acceptance gate that proves the rest actually works outside a test harness.

**Severity:** HIGH — same as the parent; this item exists so the scope cut is a tracked, named gap rather than a `RESOLVED` marker nobody can trust. **Do not mark BL-351 RESOLVED until this item, or its equivalent, is closed with a red→green test per item.**
