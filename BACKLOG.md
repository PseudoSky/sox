...
### BL-316 — `memory_recall` filtered queries silently cap at ~20 candidates per signal (`KNN_LIMIT`/`FTS_LIMIT`), regardless of the caller's `limit` or the true match count — **Open (HIGH, correctness) (2026-07-18)**

Verified directly: `memory_recall(filters:{topic:"tool-catalog"}, limit:300)` returned only 65 of the topic's 282 episodes (`memory_topics` reports `episode_count: 282` for `tool-catalog`), with no truncation indicator anywhere in the response — top-level keys were just `{results, provider_call_count}`[6][7]. Root cause, read directly: `libs/memory-core/src/recall.ts:230-231` hardcodes `const KNN_LIMIT = 20` and `const FTS_LIMIT = 20`. The vec-KNN candidate query (`:422-423`), the FTS5 candidate query (`:453-455`), and the temporal-order candidate query (`:466-467`) each correctly apply the caller's `filters` WHERE clause (per BL-100's fix landing) but then independently cap their own result rows at 20 *before* the three candidate sets are unioned and ranked — completely independent of whatever `limit` the caller actually requested. The final `ranked.slice(0, limit)` at `:655` can never return more than however many distinct rowids survived the union of three 20-row-capped channels, so a caller requesting `limit: 300` against a filter matching hundreds of episodes silently gets back a small, arbitrary fraction.

**Impact:** this is a data-integrity trap for any caller that needs a *complete* filtered result set rather than top-K relevance — e.g. an audit/retag pass over "every episode with topic X." Discovered via the memory-topic-fix job: a systematic time-windowed sweep using `memory_recall(filters:{topic:<bad_topic>})` found only 76 of 566 true matches (13%); the shortfall was caught only by cross-checking against `memory_curate op=recluster (dry_run)` — a different code path that doesn't share this cap — and independent direct SQLite queries against `~/.memory/memory.db`[8]. Any caller trusting `memory_recall`'s filter to be exhaustive, not just top-K, will silently miss the majority of matching data with zero signal.

**Fix sketch:** (a) scale `KNN_LIMIT`/`FTS_LIMIT` with the caller's requested `limit` when a `filters` clause narrows the candidate pool (e.g. `Math.max(20, limit * 2)`), since a tight filter shrinks the true candidate pool and the fixed 20-cap becomes the binding constraint instead of relevance ranking; and/or (b) surface completeness to the caller — `candidates_before_filter`/`candidates_after_filter` fields already exist internally (`recall.ts:137-138`, instantiated at `:498` and `:947-948`) but are NOT part of the actual tool response (confirmed empirically — my test's response had no such fields), so callers have no way to detect an incomplete result set. This is the same gap BL-100 flagged and its resolution did not close.

Citations: [sox-ecosystem@main, main session agent (self-verified) + memory-topic-fix forked subagent (original discovery), claude, memory-topic-fix bulk-retag data-quality task, 1: libs/memory-core/src/recall.ts:230-231 (KNN_LIMIT/FTS_LIMIT constants), 2: libs/memory-core/src/recall.ts:422-423 (vec query LIMIT), 3: libs/memory-core/src/recall.ts:453-455 (FTS query LIMIT), 4: libs/memory-core/src/recall.ts:466-467 (temporal query LIMIT), 5: libs/memory-core/src/recall.ts:655 (final slice) and :137-138/:498/:947-948 (unexposed candidate-count fields), 6: mcp__memory-server__memory_recall tool output (topic=tool-catalog, limit=300 → 65 results, no truncation flag), live call 2026-07-18T21:5xZ, 7: mcp__memory-server__memory_topics tool output (tool-catalog episode_count=282), earlier same session, 8: /Users/nix/.claude/jobs/003de74c/tmp/memory-topic-fix-checkpoint.json (76/566 = 13% recall via time-windowed memory_recall sweep, cross-verified via memory_curate recluster dry_run + direct SQLite read)]
