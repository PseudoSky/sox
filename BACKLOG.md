...
|---|---|
| **CRITICAL** | BL-447 |
| **HIGH** | BL-225, BL-284, BL-288, BL-301, BL-302, BL-319, BL-322, BL-326, BL-327, BL-334, BL-338, BL-342, BL-345, BL-349, BL-351, BL-353, BL-356, BL-358, BL-375, BL-387, BL-393, BL-394, BL-401, BL-404, BL-409, BL-413, BL-422, BL-438, BL-439, BL-440, BL-441, BL-442, BL-448 |
| **MEDIUM** | BL-99, BL-104, BL-105, BL-228, BL-274, BL-282, BL-285, BL-291, BL-296, BL-306, BL-307, BL-308, BL-312, BL-315, BL-317, BL-318, BL-328, BL-332, BL-333, BL-337, BL-341, BL-360, BL-361, BL-362, BL-378, BL-389, BL-396, BL-398, BL-400, BL-416, BL-423, BL-426, BL-432, BL-435, BL-437, BL-443, BL-444, BL-449, BL-450 |
| **LOW** | BL-103, BL-202, BL-215, BL-258, BL-261, BL-283, BL-292, BL-298, BL-305, BL-309, BL-314, BL-355, BL-379, BL-392, BL-408, BL-424, BL-436, BL-446 |
| **UNSET** | BL-163 |
...
3. **`no SQLITE_BUSY` is the wrong assertion on the production backend.** Turso is the default and its adapter sets `needsWriteSerialization: false`, so writes take `WriteQueue`'s bypass path — a path with no queue, no admission control (BL-394) and no metrics (BL-445). Asserting the *absence* of an error that the code cannot currently produce passes vacuously. The harness's value is measuring what that path actually does under concurrency: in-flight depth, latency distribution, and whether read-your-writes holds.
...
### BL-449 — The post-`VACUUM INTO` backup verdict runs ONE probe and ignores `unknown`, so a backup with a dead FTS index is certified "ok" — **Open (MEDIUM)** (2026-08-04)

**Found while:** architecting BL-341 (the 100-message cap on `backup.ts`'s integrity check) into a packet. This is a *different* hole in the same verdict and does not go away when BL-341's cap handling lands.

**Driver — two independent narrowings, both on both backends.**

1. **Probe scope.** Both adapters reverify the backup copy with `verifyStoreIntegrity(backupAdapter, { depth: 'deep', only: ['pragma_integrity_check'] })`.[1][2] `only` restricts the run to that single probe, so `fts_index_live`, `btree_index_populated`, `adapter_meta_unique`, `json_column_valid`, and `json_empty_array_null` never run against the backup. A `VACUUM INTO` copy of a store whose `idx_fts_node` is empty — **the exact live damage BL-347 shipped against** — passes with `integrityCheck: 'ok'`, because `PRAGMA integrity_check` does not read a Tantivy index and in fact only ever emits BL-360's unconditional false positive about it, which `isKnownFalsePositive()` then filters away.[3] The backup is certified healthy precisely for the artifact whose silent death cost this migration the most.

2. **`unknown` is read as healthy.** The verdict is `report.ok ? 'ok' : …`,[1][2] and `IntegrityReport.ok` is `damaged.length === 0`[4] — `unknown` findings are deliberately excluded from it, with the type's own doc-comment warning callers to *"read `unknown` separately — a probe that could not be validated is an absence of evidence, not a clean bill of health."*[5] The backup path does not read it. So when the probe cannot run at all — `probeIntegrityCheck` returns a single `status:'unknown'` finding when `PRAGMA integrity_check` throws on the backend[6] — the backup is reported `'ok'` having verified nothing whatsoever.

**Consequence.** `backupStore()` deletes the destination file and returns `E_IO` on a failed check,[7] so an operator reasonably reads a returned `BackupStoreResult` as "this backup was verified." It was verified against one pragma that is structurally incapable of seeing FTS damage, and a total failure to verify is indistinguishable from success. This is the pre-restart auto-backup path, i.e. the artifact you reach for after a crash.

**Fix sketch:** (1) drop the `only: ['pragma_integrity_check']` narrowing — run the full `deep` probe set against the backup copy, which is a throwaway read-only connection where cost is not on any hot path; if a specific probe is genuinely unaffordable, `skip` it explicitly (BL-431's lever) so the exclusion is visible rather than implied by an `only` list written before the other probes existed. (2) Treat `report.unknown.length > 0` as not-ok, or at minimum surface it: the verdict string must never be `'ok'` when a probe did not run. Both fixes are one file each and share the acceptance test.

**Acceptance (red→green, must name BL-449):** (a) seed a store with a live-but-empty FTS index (the `seedEmptyFts5Index` shape already in `integrity-selfheal.test.ts`), `backupTo()` it, and assert the result is NOT `'ok'` — fails today, since only `pragma_integrity_check` runs and it is blind to Tantivy content. (b) force the integrity probe to return `unknown` and assert the verdict is not `'ok'` — fails today, since `report.ok` ignores `unknown`.

**Severity:** MEDIUM — no corruption is caused, but it converts the one gate standing between a damaged store and a "verified" backup into a rubber stamp, on the exact damage class this program was created to chase.

**Related:** BL-341 (the cap, same verdict, different hole — fix together), BL-347 (the FTS damage this cannot see), BL-360 (why `integrity_check` says nothing useful about a Turso FTS index), BL-387 (same family: an `overall: ok` verdict blind to a whole dimension of health), BL-431 (`skip` as the visible-exclusion lever).

Citations: [wip/turso-live-metrics, architect-reviewer, claude, PKT-57 architecture, 1: libs/data/store/store-adapter/src/turso-adapter.ts:383-389, 2: libs/data/store/store-adapter/src/sqlite-adapter.ts:225-231, 3: libs/data/store/store-adapter/src/integrity.ts:1538-1540, 4: libs/data/store/store-adapter/src/integrity.ts:1696-1701, 5: libs/data/store/store-adapter/src/integrity.ts:178-189, 6: libs/data/store/store-adapter/src/integrity.ts:1553-1565, 7: libs/memory-core/src/backup.ts:195-206]
...
**ANSWERED 2026-08-05 — do not re-litigate.** The open question above ("either `memory_ping` reads a different `WriteQueue` instance, or `_trackCompletion` does not update these fields — determine which before fixing anything") is settled by reading the source, and the answer is the second, benign one: **there is no second instance.** `_trackCompletion` (`write-queue.ts:992-998`) pushes a timestamp to `_completionTimes` and increments nothing; `tasks_completed++` lives at `:1070`, inside `_processNext`, on the far side of the early return. The live observation is itself the proof — `counters` and `throughput_writes_per_sec` are returned by one `getMetrics()` call on one instance (`memory-server/src/index.ts:1055` → `metricsForPath` → `instances.get`, `write-queue.ts:980-983`), so a non-zero `throughput` means that instance did observe the writes. A different instance would have read `0` there too. Full derivation, and the eight other fields lost the same way, are filed as **BL-445**.

**⚠️ The fix sketch below is INSUFFICIENT AS WRITTEN — read BL-445 before implementing it.** Two independent reasons, both verified in source:
1. **Hoisting the size cap is a no-op.** `if (this.queue.length >= this._maxSize)` (`:767`) is expressed over a queue that is never pushed to on this path — `queue.length` is structurally `0`, so `0 >= 100` is false forever no matter where the check sits. Admission control on a natively-concurrent backend has to be expressed over **in-flight concurrency**, which the class does not currently track (`_processing` is a boolean set only in `_enqueueQueued`, `:857`).
2. **Hoisting the deadline guard is also a no-op.** Its input is `this._latencies.recentMean(...)` (`:790`) behind an `if (avgMs > 0)` (`:791`), and `_latencies` is fed only by `_recordLatencySample` at `:1069`, in `_processNext`. On the bypass path the ring is empty forever, so the guard never evaluates. **BL-445 is a hard prerequisite**, not a follow-on.

Also note the line anchors in the paragraphs above are from the pre-BL-401 file and have moved: the `_noop` gate is now `:509-511`, the early return `:657-680` (into `_runBypass`, `:700-752`), and the two admission checks `:767-782` / `:789-828` inside `_enqueueQueued`.

**Fix sketch (superseded — see PKT-58, which carries the design fork):** the *intent* stands — admission control and execution ordering must stop sharing a branch, and the reported fields must become honest (either the guards genuinely apply, or `memory_ping` reports them inactive rather than printing configured values). The *mechanism* is an open design decision, because "hoist two checks" does not survive contact with the code: what is the admission unit when there is no queue? An in-flight concurrency cap, a bounded-concurrency semaphore, deadline-shedding only, or an explicit "not applicable on this backend" declaration are four different answers with different risk profiles. **PKT-58 in `docs/reporting/memory/PLAN.md` states them as an owner decision; do not pick one silently.** Whichever is chosen, the regression test must be satisfiable on the bypass path — "saturate the queue past `_maxSize`" is not, since there is no queue to saturate.

**Severity:** HIGH — a documented safety mechanism is inert on the default backend and the health surface affirmatively reports it as active. Not CRITICAL because Turso's native concurrency means normal operation is unaffected and no data loss is implied. Related: BL-334 (status surface reports configuration, not reality), BL-154 (why serialization exists at all — re-entrancy, NOT throughput), BL-387 (same shape: a published number that no rule consumes), **BL-445 (the same early return also strands 8 of 12 metric fields, and starves this item's deadline guard of its only input — blocks this)**.
...
### BL-423 — `allocate-bl-id.mjs` resolves its target BACKLOG.md via `git rev-parse --git-common-dir`, silently writing RESERVED placeholders into the MAIN checkout even when invoked from a worktree-isolated agent session that can never commit there — **Open (MEDIUM)** (2026-08-03)

**Found while filing BL-422** during BL-349/BL-326 work, running inside a worktree-isolated session (`/Users/nix/dev/ai/sox-ecosystem/.claude/worktrees/agent-a54e5171a1615a001`). `node tools/allocate-bl-id.mjs` was invoked 3 times (expecting to reserve one id each), returning `BL-418`, `BL-419` — but grepping THIS worktree's own `BACKLOG.md` for those ids found **nothing**. The script computes `REPO_ROOT` via `git rev-parse --git-common-dir` + `path.resolve(..,'..')`, which for any worktree resolves to the **main checkout's absolute path** (`--git-common-dir` always points at the shared `.git`, by design) — so every reservation lands in `/Users/nix/dev/ai/sox-ecosystem/BACKLOG.md`, confirmed present there (`### BL-417/418/419 — RESERVED`), never in the invoking worktree's own copy.

**Why this matters.** This session's harness explicitly refuses `git -C /Users/nix/dev/ai/sox-ecosystem ...` and any git operation targeting the shared checkout from a worktree-isolated agent ("a worktree-isolated agent's git operations must target its own worktree"). So a worktree-isolated agent can reserve an id it can **never commit against** — the placeholder is permanently orphaned from that agent's perspective (it cannot fill it in, cannot delete it, cannot commit the file it landed in), and a human or a non-worktree agent has to notice and clean it up later. This is exactly the "abandoned reservation" failure mode the script's own docstring warns about, except caused by the tool itself rather than an agent forgetting to fill one in.

**Separately observed (same investigation, likely pre-existing, NOT caused by this session):** this worktree's own committed `BACKLOG.md` (HEAD, `git diff` clean) has an orphaned reservation **fragment** — a `**Driver.** Reserved by \`tools/allocate-bl-id.mjs\`...` paragraph immediately after BL-412's citations block with **no matching `### BL-41x — RESERVED` heading before it** in this branch's history (heading apparently deleted during a fill-in, trailing body left behind). Filed here rather than separately since it's the same class of defect (reservation lifecycle not fully atomic/auditable) and was found by the same grep. `check-bl-id-integrity.mjs` should be checking for this shape (orphaned reservation body without a heading), not just a live `RESERVED (RESERVED)` heading marker.

**Fix sketch:** resolve the target BACKLOG.md relative to the **invoking worktree's own working directory** (`git rev-parse --show-toplevel`, not `--git-common-dir`) when run from inside a worktree, so a worktree-isolated agent reserves and fills in an id in the file it can actually commit. If a single cross-worktree-shared BACKLOG.md is the deliberate design (plausible, given `main` is described as the source of truth across this multi-agent session), then worktree-isolated agents need either a sanctioned path to write to the main checkout's BACKLOG.md, or `allocate-bl-id.mjs` needs a worktree-aware mode that reserves locally and reconciles on merge — as shipped today, worktree isolation and this allocator are silently incompatible. Also extend `check-bl-id-integrity.mjs` to detect a `**Driver.** Reserved by...` paragraph with no preceding `RESERVED` heading (the BL-412-adjacent fragment above).

**Acceptance:** a test naming BL-423 that runs `allocate-bl-id.mjs` from a git worktree and asserts the placeholder lands in a BACKLOG.md the invoking process can subsequently read back and commit (i.e., under the worktree's own working directory, not `--git-common-dir`'s parent).

**Related:** BL-359 (the allocator's own race-fix rationale, which this defect operates orthogonally to — the lock/race logic is correct, only the target-file resolution is wrong under worktree isolation), BL-412 (the item adjacent to the orphaned fragment).

Citations: [wip/turso-live-metrics, main, claude, BL-349/BL-326 backlog filing, 1: tools/allocate-bl-id.mjs (REPO_ROOT via `git rev-parse --git-common-dir` + `path.resolve(..,'..')`), 2: /Users/nix/dev/ai/sox-ecosystem/BACKLOG.md (main checkout — `### BL-417/418/419 — RESERVED` placeholders confirmed present there, absent from the invoking worktree's copy), 3: this worktree's BACKLOG.md:2563 (orphaned `**Driver.**` fragment after BL-412, no preceding RESERVED heading, `git diff` clean against HEAD), 2026-08-03]
...
### BL-457 — `git commit --amend` commits the SHARED INDEX, so the pathspec rule has a hole exactly where it looks safest — **Open (MEDIUM)** (2026-08-05)

**Driver.** Observed live 2026-08-05. An agent ran `git commit --amend -F msg.txt` for the sole purpose of correcting a commit **subject line**. Because `--amend` was given no pathspec, it committed whatever was in the shared index: a clean 2-file / +282 commit became **8 files changed, 727 insertions, 2567 deletions**, swallowing and partially reverting `BACKLOG.md`, `CHANGELOG.md`, `PLAN.md` and `STATE.md` work staged by other agents.

Recovered without loss by `git reset --soft <good-sha>` — which moves HEAD only and leaves the index and worktree untouched — and verified afterwards.

**Why the existing rule does not cover it.** `CLAUDE.md`'s constraint is worded as *"never `git add -A`, `git add .`, or a bare `git commit` after `git add`"*, and `tools/commit-mine.mjs` exists for the contended-file case. Every example is about **staging**. `--amend` reads as a message-only operation — the agent's stated intent was literally "fix the subject" — so the rule does not appear to apply, and the failure is quieter than a bare commit because the resulting SHA still looks like the commit you just made.

**Why it is worse than a bare `git commit`.** A bare commit produces an obviously large new commit. `--amend` produces a *replacement* for a commit that was already reviewed and reported, so the swallowed work hides behind a subject line someone has already read and approved.

**Fix candidates, none chosen:** (a) extend the `CLAUDE.md` pathspec rule to name `--amend` explicitly, with `git reset --soft` recorded as the recovery; (b) teach `commit-mine.mjs` an amend mode that rewrites only the message via `commit-tree`, never touching the index; (c) a `pre-commit` guard that refuses a pathspec-less `--amend` when the shared index is non-empty.

**Related:** BL-409 (shared index; this is the same hazard through a door the rule leaves open), BL-456 (`nx test` rebuilding from uncommitted source), BL-422 (commits landing on disposable worktree branches).

Citations: [wip/turso-live-metrics, impl-writepath + claude, claude, PKT-64/BL-445, 1: the live incident — `git commit --amend -F msg.txt` turning a 2-file/+282 commit into 8 files/+727/-2567, recovered via `git reset --soft`, 2: CLAUDE.md pathspec constraint, worded entirely in terms of `git add`/bare `git commit`, 3: tools/commit-mine.mjs (the sanctioned contended-file path, which has no amend mode)]
---
