...
### BL-463 — an agent that finishes or is stopped while holding staged files leaves a revert bomb in the shared index — **Open (MEDIUM)** (2026-08-05)

**Driver.** Three separate occurrences on 2026-08-05, all found by accident rather than by any guard:

| # | Staged content vs HEAD | Discovered by |
|---|---|---|
| 1 | `STATE.md` 50 lines behind HEAD | `commit-mine.mjs`'s "staged by someone else" notice |
| 2 | six paths, **2,687 deletions**, `CHANGELOG.md` at −260 | an agent warning that `git diff` was misleading |
| 3 | four paths, `CHANGELOG.md` at −161, `BACKLOG.md` −292/+174 | regenerating `plan-status` after the holder stopped |

In every case the working trees were correct and **only the index was stale**, so `git status` showed nothing alarming and the files looked fine on disk. A bare `git commit` by any agent would have reverted committed work in bulk.

**⚠️ ROOT CAUSE CORRECTED 2026-08-05 — read this before acting on the paragraph below.** This item originally concluded "lifecycle, not commit mechanics" and named `tools/commit-mine.mjs` as *immune*. That attribution is **wrong**, and it was written by the same session that then reproduced the defect six times. **`commit-mine.mjs` is a generator of this condition on every run** — see **BL-465**. It seeds a PRIVATE index from HEAD (`GIT_INDEX_FILE`, `:141-143`), writes a tree, and moves the branch with `update-ref` (`:160`); nothing ever resyncs the SHARED index, so the instant HEAD advances the shared index still holds the pre-commit blobs for every path committed. `git diff --cached` then reads as the exact inverse of the commit. Because `commit-mine` is the *mandated* path for the hottest files, compliance is the vector: the more faithfully an agent follows the rule, the more bombs it plants. Incident #1 below (`STATE.md` 50 lines behind HEAD, found by commit-mine's own notice) carries exactly that signature and was probably self-inflicted rather than orphaned. **The incidents below should be re-examined for how many were caused by the remedy.** Interim mitigation, verified: run `git restore --staged <paths>` immediately after every `commit-mine` invocation.

**The lifecycle vector is real but secondary.** `git add` is how the entries get there, but the hazard is that **stopping or completing an agent does not clean up its index entries**. They outlive the agent, silently, and the next agent to run a pathspec-less commit inherits them. `tools/commit-mine.mjs` is immune by design — it seeds a private `GIT_INDEX_FILE` from HEAD — which is why it kept reporting "N path(s) staged in the SHARED index by someone else" while the entries accumulated unnoticed.

**Second-order effect, observed:** plain `git diff` compares against the index, not HEAD, so while a stale index is present `git diff` actively misleads. One agent nearly mis-scoped a commit on it and said so; the correct habit is `git diff HEAD`.

**Distinct from BL-409**, which governs *what goes into a commit* given a contended index. This is about entries persisting after their owner is gone — the fix is at dispatch/teardown, not at commit time.

**Fix candidates, none chosen:** (a) an orchestrator teardown step that runs `git restore --staged` for any path an agent staged but never committed; (b) a `pre-commit` guard refusing a pathspec-less commit whose index diverges from HEAD by more than N lines; (c) worktree isolation per agent, which removes the shared index entirely and is already the dispatch default — these three all happened in a shared non-worktree checkout.

**Recovery, verified three times:** `git restore --staged <paths>` resets index entries to HEAD and leaves every working-tree file untouched. Nothing was lost in any of the three incidents.

**Related:** BL-409 (shared index at commit time), BL-457 (`--amend` commits the shared index), BL-422 (commits stranded on disposable worktree branches), BL-456 (`nx test` rebuilding from uncommitted source).

Citations: [wip/turso-live-metrics, claude, claude, session orchestration, 1: `git diff --cached --stat` at three points on 2026-08-05 showing the three divergences above, 2: tools/commit-mine.mjs (private GIT_INDEX_FILE, the reason it is immune and the reason it kept reporting the condition), 3: recovery via `git restore --staged`, working trees verified unchanged each time]
---
...
### BL-465 — `commit-mine.mjs` leaves the shared index behind HEAD by exactly the commit it just made, so the sanctioned remedy for BL-463 MANUFACTURES a BL-463 revert bomb on every run — **Open (HIGH)** (2026-08-05)

**Driver — reproduced deliberately, start to finish, in one session 2026-08-05.**

| step | `git diff --cached --stat` | note |
|---|---|---|
| before commit | **empty** | shared index clean, identical to HEAD `f7abd50` |
| `node tools/commit-mine.mjs -m … -- BACKLOG.md PLAN.md STATE.md` | — | reported `committed da3c1880a` |
| immediately after | **3 files, +12 / −247** | shared index now **247 lines behind** the new HEAD |
| `git restore --staged <the 3 paths>` | **empty** | recovered, working tree untouched |

The index was **clean before the commit**, so this divergence was not inherited from another agent
— the tool created it.

**Mechanism.** `commit-mine` builds its tree in a private `GIT_INDEX_FILE` seeded from HEAD and moves
the branch with `commit-tree` + `update-ref`.[1] It deliberately never writes the shared index — that
is the whole design, and it is what makes the tool immune to another agent's staged content. But
`update-ref` advances HEAD **without refreshing the shared index's entries for the paths just
committed**, so every such path is left in the shared index holding the *old* HEAD's blob. The index
is not stale because someone abandoned it; it is stale because the branch moved out from under it.
The condition is created on **every successful run**, for **every path** in the commit.

**Why this is HIGH rather than a footnote.** `commit-mine` is the *mandated* path for the contended
files — `CLAUDE.md` names it for `BACKLOG.md`/`CHANGELOG.md`/`PLAN.md`, and agent task briefs
routinely require it. So **compliance is the vector**: the more faithfully agents follow the rule, the
more reliably revert bombs get planted in the shared index, on precisely the hottest files. The next
agent to run a pathspec-less `git commit` — or a pathspec-less `git commit --amend`, which is
BL-457 — commits that stale tree and reverts the work. `git diff` (which compares against the index,
not HEAD) actively misleads in the window, exactly as BL-463 records.

**This materially revises BL-463's root-cause finding.** BL-463 concludes *"Root cause is lifecycle,
not commit mechanics … the hazard is that stopping or completing an agent does not clean up its index
entries."* That is at most partly right. At least one vector **is** commit mechanics, and it is the
tool BL-463 credits as immune: BL-463's own incident #1 (`STATE.md` 50 lines behind HEAD, *"discovered
by `commit-mine.mjs`'s 'staged by someone else' notice"*) has exactly this signature — a single file,
behind HEAD, in a checkout where `commit-mine` was in use. Its incidents should be re-examined for how
many were self-inflicted by the remedy rather than left by stopped agents; the two causes are
indistinguishable after the fact, which is why this went unnoticed.

**Fix sketch — narrow, and it must not become the bug it is fixing.** After `update-ref` succeeds,
resync the shared index **only** for the committed paths, and **only** where that path's shared-index
entry still matches the *old* HEAD blob (i.e. no other agent had staged content there):
`git update-index --cacheinfo` / `git restore --staged <path>` per qualifying path. Where another
agent *has* staged different content, **leave it alone and print a warning naming the path** —
overwriting it would destroy exactly the work this tool exists to protect, converting a revert bomb
into immediate data loss. A blanket `read-tree HEAD` against the shared index is **wrong** for the
same reason and must not be used.

**Acceptance (red→green, must name BL-465):** a test that seeds a clean shared index, runs
`commit-mine` over a tracked file, and asserts `git diff --cached` is **empty** afterwards — red
today, showing the full inverse of the commit. Plus the protective arm: with another agent's content
staged for one of the committed paths, that path's index entry is left **byte-identical** and the run
warns, rather than being reset.

**Interim mitigation, verified:** run `git restore --staged <the paths you just committed>`
immediately after every `commit-mine` invocation. Working-tree files are untouched — confirmed in the
reproduction above, with the committed content re-verified present in HEAD afterwards.

**Related:** BL-463 (the revert-bomb class; this is a *manufacturing* source it does not account for),
BL-409 (the shared index at commit time — the problem `commit-mine` was built to solve), BL-457
(`--amend` commits the shared index — the loudest way this one detonates), BL-422.

Citations: [wip/turso-live-metrics, architect-reviewer, claude, unscheduled-item planning pass, 1: tools/commit-mine.mjs:140-160 (private `GIT_INDEX_FILE` at :141, `read-tree HEAD` at :142, `update-ref` at :160 — no shared-index resync anywhere after it), 2: tools/commit-mine.mjs:29-31 (the documented "shared index is never written" guarantee whose side effect this is), 3: live reproduction 2026-08-05 — `git diff --cached --stat` empty before `commit-mine`, `3 files changed, 12 insertions(+), 247 deletions(-)` immediately after committing `da3c1880a`, empty again after `git restore --staged`, 4: BACKLOG.md BL-463 incident table, row 1]
