...
Regenerated 2026-08-06: **89 open** (BL-437 resolved 2026-08-06; BL-426 resolved 2026-08-06, see CHANGELOG.md).

| Priority | Open items |
|---|---|
| **CRITICAL** | BL-447 |
| **HIGH** | BL-225, BL-284, BL-288, BL-301, BL-302, BL-319, BL-322, BL-326, BL-327, BL-334, BL-338, BL-342, BL-345, BL-349, BL-351, BL-353, BL-356, BL-358, BL-375, BL-387, BL-393, BL-401, BL-404, BL-409, BL-413, BL-422, BL-438, BL-439, BL-440, BL-441, BL-442, BL-448, BL-468 |
| **MEDIUM** | BL-99, BL-104, BL-105, BL-228, BL-274, BL-282, BL-285, BL-291, BL-296, BL-306, BL-307, BL-308, BL-312, BL-315, BL-317, BL-318, BL-328, BL-332, BL-333, BL-337, BL-378, BL-389, BL-396, BL-398, BL-400, BL-416, BL-423, BL-432, BL-435, BL-443, BL-444, BL-450, BL-461, BL-462, BL-469 |
...
### BL-472 — `WriteQueue.clearInstances()`/`closeAllForShutdown()` close the adapter without draining fire-and-forget Phase-B embed work, so a shutdown mid-write discards a just-computed embedding as an E_IO error — **Open (MEDIUM)** (2026-08-06)

**Found while diagnosing D1** (`turso-clean-room.test.ts` / `clustering-e2e.test.ts` "vec_node stays empty" reports — root cause of THOSE two files was unrelated and is fixed in this same session, see below). While tracing why the reported stderr showed `[memory-core embed-pipeline] Phase-B FAILURE … {"code":"E_IO","message":"The database connection is not open"}` followed by an identical `heal FAILURE`, I found a real, separate, live gap in the shutdown path, not exercised by either failing test's assertion but real in production.

**Mechanism.** `memory_write`'s async-default path schedules Phase B (`schedulePendingEmbeds`, off the WriteQueue slot) as fire-and-forget — `schedulePhaseBAndWake` (index.ts:2822-2838) calls `void schedulePendingEmbeds(...)`, never awaited, and additionally arms a debounced (`DEFAULT_DRAIN_WAKE_DEBOUNCE_MS` = 250 ms) background drain pass via `wakeDrain('write')` that itself runs `healMissingVectors`-shaped work. Neither `WriteQueue.clearInstances()` (write-queue.ts:451-459, test teardown) nor `WriteQueue.closeAllForShutdown()` (write-queue.ts:502-539, the production `coordinatedShutdown` path in backend.ts:223-289) waits for `flushPendingEmbeds()` (embed-pipeline.ts:500-504, which exists for exactly this purpose and is already called by six other memory-core/memory-server spec files) before calling `q.adapter.close()`. If Phase B (or the debounced heal-shaped drain) is still mid-flight when the adapter closes, its `wq.enqueue(...)` / `embed()` retry against the now-closed connection throws `E_IO: The database connection is not open` — the embedding was computed (CPU/ONNX cost already paid) and then discarded, logged only to stderr.

**Why this is a real production gap, not just a test artifact.** `backend.ts`'s own `coordinatedShutdown` doc comment (BL-405) already documents one prior instance of this exact class — "database connection is not open" fired 11 times in live telemetry on 2026-08-03 from the WP-5 idle-checkpoint timer racing shutdown, fixed by cancelling that timer first. Phase-B's fire-and-forget embed work is the same race, unfixed: `coordinatedShutdown` step 1 (`terminateEmbedWorkers()`) kills the shared fastembed child before step 2 closes the adapters, so an in-flight Phase-B `embed()` call can fail on the IPC side too, independent of the adapter-close race.

**Not silent data loss in the strict sense** — `embedBacklogStats()`/`healMissingVectors()` on the NEXT process restart will pick up the missing vector (this is the documented FAILURE/RETRY contract in embed-pipeline.ts:48-56) — but it is unnecessary log noise on every graceful restart under write load, wasted embed compute, and one bounded-race away from being clean: a shutdown that briefly (with its own timeout, bounded well inside `SHUTDOWN_SAFETY_NET_MS`) awaits `flushPendingEmbeds()` before `terminateEmbedWorkers()`/`closeAllAdapters()`/`WriteQueue.closeAllForShutdown()` would let in-flight Phase-B work land cleanly instead of failing and waiting for the next process's heal tick.

**Not fixed here** — deliberately out of the D1 scope (embed-pipeline.ts/write-queue.ts cannot import each other; the drain has to happen one layer up, in `backend.ts`'s `coordinatedShutdown` and/or `WriteQueue.clearInstances()`'s test-teardown twin) and needs a bounded-race design (an unbounded `flushPendingEmbeds()` await could stall shutdown for up to the 240 s `SOX_EMBED_HEAL_TIME_BUDGET_MS` if a heal pass happens to be running, which would itself blow `SHUTDOWN_SAFETY_NET_MS` and defeat the fix) plus its own red→green regression test. Filed rather than folded into the D1 fix per the no-defer rule.

Citations: [wip/turso-live-metrics, debugger, claude, D1 task, 1: libs/memory-core/src/write-queue.ts:451-459 (`clearInstances` closes `q.adapter` unconditionally), 2: libs/memory-core/src/write-queue.ts:502-539 (`closeAllForShutdown`, same gap, production path), 3: extensions/bundles/sox-memory-bundle/members/memory-server/src/index.ts:2822-2838 (`schedulePhaseBAndWake`, fire-and-forget `void schedulePendingEmbeds(...)` plus debounced `wakeDrain('write')`), 4: extensions/bundles/sox-memory-bundle/members/memory-server/src/backend.ts:223-289 (`coordinatedShutdown` — step 1 `terminateEmbedWorkers()`, step 2 `closeAllAdapters()`, step 2b `WriteQueue.closeAllForShutdown()`, none preceded by a Phase-B drain), 5: libs/memory-core/src/embed-pipeline.ts:500-504 (`flushPendingEmbeds` — the existing seam, unused by any shutdown path), 6: libs/memory-core/src/write-queue.ts:488-501 (BL-405's own doc comment naming the identical "database connection is not open" race against the WP-5 checkpoint timer, 11 live occurrences 2026-08-03)]
---
