...
### BL-469 — `test-bl266`'s skipped invariants report `[PASS]` and count toward "ALL 5 INVARIANTS PASS" — **Open (MEDIUM)** (2026-08-05)

**Driver.** `tools/test-bl266-bundle-invariants.mjs` gates its (c) atomicity, (d) typecheck and (e)
checksum-stability arms on `--build-cmd`/`--source`/`--rebuild-cmd`. When those flags are absent each
arm calls `report(name, true, '(skipped — …)')` — i.e. it reports **`[PASS]`** — and the run then
prints `ALL 5 INVARIANTS PASS` having actually verified two.[1] Run read-only against the real
memory-server dist it exits 0 and claims five green invariants; three were never executed.[2]

**Why it matters.** This is the BL-167 pattern that AGENTS.md calls out by name — a check that
presents as green for the case it declined to test. It is materially worse in a harness: the natural
default invocation of this guard is the read-only one (the flagged form mutates source and shells to
an arbitrary build command), so the *common* path is the one that overstates its own coverage 5-vs-2.
Any runner that consolidates these guards and sums their reported assertions inherits the
overstatement and scales it.

**Fix sketch (not applied — this item deliberately does not weaken the existing assertions).** Give
`report()` a third state and print `[SKIP]`, excluded from the pass tally, with a summary line of the
form `2 passed, 3 skipped`; a skipped invariant must not be able to make a run read as fully verified.
Whether an all-skip run should additionally exit non-zero under some `--strict` is the open question —
it would make the guard honest by default but changes its exit contract, so it belongs with the
BL-466 harness spec rather than being decided here.

**Related:** BL-466 (the harness that would consume this count), BL-167 (a guard that skips the
failing case), BL-225 (a marker is not evidence).

Citations: [wip/turso-live-metrics, test-automator, claude, BL-466, 1: `tools/test-bl266-bundle-invariants.mjs`:113, 142, 172 — each early-returns via `report(..., true, '(skipped …)')`, 2: `node tools/test-bl266-bundle-invariants.mjs --outdir extensions/bundles/sox-memory-bundle/members/memory-server/dist --externals better-sqlite3,sqlite-vec,fastembed,onnxruntime-node,@tursodatabase/database` → 5 `[PASS]` lines, 3 of them `(skipped …)`, then `ALL 5 INVARIANTS PASS`, measured 2026-08-05]
...
### BL-471 — the fastembed advisory-lock format is now parsed in two files with no shared constant and no guard tying them together — **Open (LOW)** (2026-08-06)

**Driver.** BL-432's `competing_host_pid` field needs the lock file that `fastembedProcessHost.ts` writes, but it is read from `sharedFastembedProcess.ts`, which deliberately does **not** import the host module — that file registers `process.on('message')` and calls `checkAndClaimFastembedLock()` at module scope, side effects meant for the forked child and unsafe to run in the parent.[1] The consequence is that the default lock path and the `{pid, startedAt}` payload shape are now spelled out **independently in both files**: `fastembedProcessHost.ts:111` and `sharedFastembedProcess.ts:84` each compute `process.env['SOX_FASTEMBED_LOCK_PATH'] ?? join(tmpdir(), 'sox-fastembed-host.lock')`, and the reader duck-types `raw.pid`/`raw.startedAt` against a writer it never references.[2]

**Why it matters.** The duplication is not itself a bug — avoiding the import is correct, and the reader is defensively wrapped so a malformed or absent lock file degrades to `null` rather than breaking an embed. The defect is that **nothing fails when the two drift.** Change the lock filename or rename `startedAt` in the host, and `competing_host_pid` silently becomes permanently `null` — which reads exactly like *"no competing host was present"*, the single most load-bearing label BL-432 added. That is this repo's recurring shape: an absent signal indistinguishable from a negative finding (BL-379, BL-449, BL-431, BL-469).

The window is not hypothetical. `fastembedProcessHost.ts` was edited **the same day** this reader was written, by BL-426's shutdown fix.[3] That change did not touch the lock logic — but nothing would have told anyone if it had.

**Fix sketch, in preference order.** (a) Extract the path resolver and the payload type into a small side-effect-free module both files import — the real fix, and it is a handful of lines, since the side-effect hazard lives in the host's module scope and not in this logic. (b) Failing that, a test that has the host actually write a lock file and the reader parse it, so the contract is executable rather than commented. Do **not** simply add a comment cross-referencing the two.

**Severity:** LOW — the failure mode is a silently-degraded telemetry label, not incorrect embedding or data loss.

Citations: [wip/turso-live-metrics, plan-orchestrator, claude, PKT-83 follow-up, 1: libs/data/embed/embedding-provider/src/sharedFastembedProcess.ts:76-84 (the comment recording why the import is avoided), 2: libs/data/embed/embedding-provider/src/fastembedProcessHost.ts:108-111 vs sharedFastembedProcess.ts:82-91, 3: commit 263c8d1 (BL-426) modified fastembedProcessHost.ts on 2026-08-06, the same day 631edfb added the duplicated reader]
