...
> registry + `sox list --all`), `94e7327` P5 (stale GC), `3a95564` P6 (daemon mode),
> `dbe9439` P7 (health surface / `sox status`), `24cb5fe` P8 (bundle co-location + visibility),
> `dc56cc1` P9 (global install registry). Deliverable files `lock.ts`, `log-manager.ts`,
> `gc.ts`, `registry.ts` are committed; `sox status/--all/--daemon/logs` are wired in `main.ts`.
...
The sox host runtime was built to satisfy the EIM plan's DoD (service lifecycle, permission
...
There is no machine-wide registry of running sox supervisors. `sox list` reads one
`runtime.json` for one scope+directory. Five simultaneous `sox start --scope=project`
...
point; `sox list --all` is not implementable without a global index.
...
**Impact:** `sox list` can report RUNNING for a process that died hours ago. Any
...
### 3. Concurrent `sox start` for the same scope+directory races

The idempotent check in `startRuntime()` reads `runtime.json` and bails if it exists.
Two simultaneous `sox start` invocations can both pass that check before either has
...
rotation, no `sox logs --id=<ext>` command, no structured log format.

**Impact:** production debugging requires attaching to the terminal that ran `sox start`.
...
sox spawns one child process per extension (the entrypoint). If that process spawns
workers, sox has no visibility into them. On `sox stop`, only the direct child receives
`SIGTERM`. Workers are orphaned — they continue running, potentially holding file locks,
ports, or database connections.

**Impact:** `sox stop` does not actually stop the service in the presence of workers. The
...
**Impact:** `sox stop` is advisory, not authoritative. Orphaned processes accumulate.
...
there is no `sox status --id=<ext>` command that combines pid liveness + socket
...
|-------------|-------------------|
| **R1 — global enumeration** | `sox list --all` returns every running supervisor on the machine with accurate state, regardless of which directory started it |
| **R2 — self-healing state** | Any `sox list` or `sox status` call validates reported state against OS reality before returning it; stale entries are removed |
| **R3 — safe concurrent start** | Two simultaneous `sox start` invocations for the same scope+directory are serialised; exactly one supervisor starts |
| **R4 — log persistence** | Every extension's stdout+stderr is written to a rotating log file; `sox logs --id=<ext> [--follow]` streams it |
| **R5 — complete stop** | `sox stop` terminates the extension's entire process tree (direct child + workers); SIGTERM is followed by SIGKILL after `stop_timeout_ms` |
...
| **R7 — live health surface** | `sox status --id=<ext>` returns a structured, reality-verified health record (pid alive, socket reachable, last tool-call latency, log tail) |

---

---

### 9. No daemon mode — `sox start` occupies the terminal

`sox start` keeps the parent process alive and attached to the terminal. There is no
...
`sox stop` from the same directory.

**Impact:** `sox start` cannot be used in scripts, CI, or anything that expects the
...
- **Authoring**: `sox init bundle` scaffolds the bundle manifest but does not co-locate
...
- Members are invisible to `sox search` and `sox install` by default
  (`visibility: internal` or equivalent); only the bundle entry is surfaced.
- A direct `sox install memory-server` or `sox start memory-server` produces a helpful
  error: "memory-server is part of sox-memory-bundle — install the bundle instead."
- `sox start <bundle-id>` / `sox stop <bundle-id>` operate atomically on all members.
...
| **R6 — signal contract** | The framework defines and enforces a minimum signal-handling contract for extensions; the authoring scaffold generates a compliant handler |
| **R7 — live health surface** | `sox status --id=<ext>` returns a structured, reality-verified health record (pid alive, socket reachable, last tool-call latency, log tail) |
| **R8 — daemon mode** | `sox start --daemon` detaches the supervisor, writes logs to a managed file, prints the log path, and returns exit 0 to the caller; `sox logs --id=<ext> [--follow]` streams the log |
| **R9 — bundle co-location and visibility** | Bundle members live under their bundle's directory; they are hidden from search/install by default; direct member install/start produces a helpful redirect; `sox start/stop <bundle-id>` is atomic |
...
3. The concurrency model for `sox start` (lockfile, flock, atomic rename, or other)
4. Log pipeline architecture (pipe routing, file format, rotation policy, streaming API)
5. Daemon mode: how `--daemon` detaches, where logs go, how `sox logs` streams them
...
9. The `sox status` data model and its API surface
...
12. The atomic start/stop protocol for bundles: whether the supervisor treats the bundle
    ...
    process level
13. Authoring changes: how `sox init bundle` co-scaffolds members, what the generated
    directory layout looks like, and how `sox validate` enforces co-location
