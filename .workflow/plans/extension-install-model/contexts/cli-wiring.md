...
- **Reference Pattern:** `bin/soxe` is the shell shim that invokes `apps/sox/src/main.ts`. `apps/sox/scripts/rewrite-paths.cjs` handles `@adhd/sox-*` resolution at runtime. Existing verb dispatch (e.g. `start`, `list`, `stop`) shows the pattern to follow.

- **Delta Spec:**
  - `bin/soxe` — ensure it invokes the built `apps/sox` bundle with no bare module references; `rewrite-paths.cjs` must be called before any `@adhd/sox-*` import.
...
- **Invariants:** `[inv:tier3-proof]` — the guard drives real `node bin/soxe <verb>` calls. `[inv:sandbox-isolation]` enforced by `probe_done`. Merge protocol: this state writes `bin/soxe` + `main.ts` FIRST; `lifecycle` rebases onto this state's output.
...
- [ ] **[cli-wiring.1]** `soxe install <agent> --host claude --scope project` exits 0 and places the file (real CLI).
      Via guard: `bash .workflow/plans/extension-install-model/scripts/guards/cli-wiring.sh`
- [ ] **[cli-wiring.2]** `soxe install <agent> --host=claude --scope=project` (equals syntax) exits 0 and places the file.
      Via guard (equals-flag test case).
- [ ] **[cli-wiring.3]** `soxe build <mcp-server>` exits 0 (verb is wired).
      Via guard.
- [ ] **[cli-wiring.4]** `soxe diff <agent>` exits 0 on a clean install (new verb is wired; no drift reported).
      Via guard.
- [ ] **[cli-wiring.5]** `soxe update <agent>` exits 0 (verb is wired).
      Via guard.
...
**Merge protocol:** This state writes `bin/soxe` + `apps/sox/src/main.ts` FIRST. `lifecycle` (a later state) rebases onto this state's output when it adds `disable`, `enable` wiring.

---

## Contract Promise

- **Modified:** `apps/sox/src/main.ts` — wires `install`, `build`, `diff`, `update`; fixes flag parser; `rewrite-paths.cjs` — verified correct; `bin/soxe` — verified no bare module leaks
...
- [ ] **After all verbs wired** — commit `main.ts` + `bin/soxe` + `rewrite-paths.cjs`:
...
- The `rewrite-paths.cjs` step must run at build time so the emitted `bin/soxe` bundle has no `@adhd/sox-*` bare specifiers at runtime. If the paths are already resolved by esbuild, that is fine; just verify with a grep on the built artifact.
