...
# rehome-memory-server — Collapse memory-server onto `@sox/mcp-runtime`
...
After this state `memory-server` is re-homed onto `@sox/mcp-runtime`: its hand-rolled MCP loop and
...
- **Primitive:** REWRITE `extensions/mcp-servers/memory-server/src/index.ts` onto `@sox/mcp-runtime`;
...
- **Reference Pattern:** spec §8 final bullet ("memory-server's MCP loop + vendored
  `compilePolicyFromEnv` collapse into `@sox/mcp-runtime`"); the current
...
- **Delta Spec:**
  ...
  - **Delete** the vendored `checkDbPathPolicy` and `getPolicy`; the wrapper's ctx policy accessors
    ...
    used by `@sox/mcp-runtime`).
  - `extension.json` declares `@sox/mcp-runtime` and derives `serves`.
...
- [ ] **[rehome-memory-server.1]** memory-server uses `serve()` from `@sox/mcp-runtime`, not a
      hand-rolled loop. `grep -nE 'serve|@sox/mcp-runtime' extensions/mcp-servers/memory-server/src/index.ts` → non-empty.
...
- [ ] **[rehome-memory-server.4]** `extension.json` declares `@sox/mcp-runtime`.
...
- **Added:** `@sox/mcp-runtime` dependency on memory-server; `serve()`-based tools.
...
- [ ] **After the guard (memory tests + e2e) is green** (mandatory) — commit source **and**
      `state.json` / `dag.json`: `refactor(eim): rehome-memory-server complete — onto @sox/mcp-runtime, vendored guard gone — guard green`
