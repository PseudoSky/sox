...
pnpm exec nx run sox:build >/dev/null 2>&1; rc=$?; [ $rc -eq 0 ] || { echo "FAIL: soxe build"; exit 1; }
node -e "const {validate}=require('./libs/manifest/dist/index');process.exit(validate(require('./apps/sox/extension.json')).ok?0:1)"; rc=$?; [ $rc -eq 0 ] || { echo "FAIL: soxe extension.json invalid (D1)"; exit 1; }
node -e "process.exit(require('./apps/sox/extension.json').type==='command'?0:1)"; rc=$?; [ $rc -eq 0 ] || { echo "FAIL: soxe type not command (D2)"; exit 1; }
