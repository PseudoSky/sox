...
node dist/apps/sox/main.js init hook smoke-hook --events SessionEnd --runtime shell --out "$T" >/dev/null 2>&1; rc=$?; [ $rc -eq 0 ] || { rm -rf "$T"; echo "FAIL: A1 init"; exit 1; }
node -e "const {validate}=require('./libs/manifest/dist/index');process.exit(validate(require('$T/smoke-hook/extension.json')).ok?0:1)"; rc=$?; rm -rf "$T"; [ $rc -eq 0 ] || { echo "FAIL: A1 scaffolded manifest invalid"; exit 1; }
