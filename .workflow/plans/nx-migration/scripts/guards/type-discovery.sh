...
node -e "const {validate}=require('./libs/manifest/dist/index');process.exit(validate(require('./apps/sox/extension.json')).ok?0:1)"; rc=$?; [ $rc -eq 0 ] || { echo "FAIL: schema change not additive (soxe manifest broke)"; exit 1; }
