...
`CONTRACTS.md` (the `@sox/memory-enrich` API + v1 MCP tool contracts)
...
## §P2 — `@sox/memory-enrich` package (deterministic enrichment surface)

Create `libs/memory-enrich/` (`@sox/memory-enrich`) implementing `CONTRACTS.md` C1: `enrichOnWrite`,
