...
- `processBatchEnrich` (`memoryd.ts:286-300`) calls `runBatchEnrich(this.db)` from `@sox/memory-enrich`;
...
`@sox/memory-core` (the C7 reuse pattern this repo already enforces) rather than carrying a forked
