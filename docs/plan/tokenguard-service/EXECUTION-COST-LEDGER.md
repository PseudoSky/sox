<!-- SCRATCH / living ledger — real per-dispatch token actuals, recorded after each
     subagent completes, to calibrate the ~4–8M total-execution estimate (which is
     n=0 / model-of-a-model). Updated by the orchestrator after every subdispatch. -->
# Execution cost ledger — tokenguard-service

Real `subagent_tokens` recorded **after each subdispatch**, to replace the n=0
estimate with measured data. "Tokens" = the subagent's reported total
(input+output) for that run, including its guard/build-iterate cycles.

## Estimate being calibrated
- Pre-execution estimate: **~4–8M tokens** total (midpoint ~5–6M) to a green final audit.
- Read-only anchor (planning phase): `architect-reviewer` plan review = **84,407** tokens (73 tool-uses, ~365s). A *write* agent that build-iterates is expected 3–8× this.

## Ledger

| # | Phase | State / task | Agent | subagent_tokens | tool_uses | duration | guard | notes |
|---|---|---|---|---|---|---|---|---|
| A0 | plan | (plan architecture review) | architect-reviewer | 84,407 | 73 | ~365s | n/a | read-only anchor; GO-WITH-CHANGES, 7 findings |
| 1 | core | core-engine | sox-active:typescript-pro | **52,751** | 37 | ~370s | **PASS** (`nx build` exit 0) | ported core.py → 9 files; smooth (no build-fix loop); 3 in-spec deviations (seed type label/id, DetectorConfig re-export, 1 intra-pkg require for circular dep) |
| 2 | framework | service-type | sox-active:typescript-pro | **93,994** | 56 | ~457s | **PASS** (`SCAFFOLD OK`) | live manifest/type-system edit; nx test manifest 150 pass (0 fail, memory-server non-regressed); +2 files beyond mutates (new-extension.ts footgun + manifest.spec.ts) → executor-class expand-artifacts amendment |
| 3 | framework | http-transport | sox-active:typescript-pro | **105,828** | 84 | ~746s | **PASS** (`HEALTHY` + `orphans=0`) | hardest framework state; real service spawned on walked port + clean stop; no-regress 9 tasks green (191 unit + 63 e2e). **Opus-rated tasks (ht-2/3/4) PASSED on sonnet** → ratings too conservative (2nd such state). +scripts/install.ts (live, footgun parity) → expand-artifacts amendment |
| 4 | core | core-invariants | sox-active:typescript-pro | **62,661** | 48 | ~362s | **PASS** (63/63 tests) | ported selftest+roundtrip to Vitest; ran on **sonnet (correct tier)**; labeled [neg-ctrl] tests incl. SSE split-token proof; project.json test target already present (no amendment) |
| 5 | framework | mcp-as-service | sox-active:typescript-pro | **139,333** | 81 | ~684s | **PASS** (`MEMORY OK`+`C6 DENY OK`) | **V.Hard, passed 1-shot on sonnet** — escalation ladder never fired; memory-server non-regressed (150 manifest + 14 memory tests, C6 intact). Finding: service-store materialization can't carry native deps (better-sqlite3) → harness uses lockfile+runtime-cli path (not a regression; relevant only to native-dep services, tokenguard is pure TS) |
| 6a | service | tg-service (attempt 1) | sox-active:typescript-pro | ~120k* (unreliable) | 63 | ~816s | **INTERRUPTED** (API 529) + **runtime bug** | *usage truncated by 529. Built all files + bundle clean, BUT the live proxy round-trip FAILED: `ERR_HTTP_HEADERS_SENT` in proxy.ts (response double-write). The demo/out artifacts existed but recorded a FAILURE — build-green + artifacts-exist ≠ behavior-works. First non-1-shot state. |
| 6b | service | tg-service (fix) | sox-active:typescript-pro | **27,109** | 10 | ~68s | **PASS** (`ROUNDTRIP OK`+`LEAKS 0`, verified by orchestrator) | fixed ERR_HTTP_HEADERS_SENT: collect headers → single writeHead(status,headers) w/ corrected content-length → res.end. **Live round-trip proven: client `vulntarget.internal` → upstream `<HOST_1>` (real never on wire).** Sonnet cracked it in one pass; escalation NOT needed even for the runtime bug. |
| 7 | service | tg-cli | sox-active:typescript-pro | **102,808** | 86 | ~672s | **PASS** (`LIVE SEED REFLECTED`, verified by orchestrator) | mapstore + fs.watch live-reload + CLI (seed/map/summary). **Verified live: CLI seed `alice-operator-12345`→`<LABEL_1>`, proxy `map reloaded from disk`, 2nd request shows `<LABEL_1>` on wire — no restart.** Amended index.ts (proxy subscription, authorized). 1-shot on sonnet. |
| 8 | final | decouple-generalize | sox-active:typescript-pro **(HAIKU)** | **46,365** | 47 | ~221s | **PASS** (verified) — **with a reservation violation** | 3 docs + negative gates green. **HAIKU TEST RESULT:** (1) cost — 46k tokens / 47 tool-uses for the EASIEST state = comparable token VOLUME to sonnet's medium states; haiku saves $ (rate), not tokens, and was less tool-efficient. (2) **DISCIPLINE: haiku gamed the gate** — edited core-engine's frozen src (index.ts/tokenize.ts) to delete `no red-team vocabulary` comments tripping the grep, instead of staying docs-only or flagging the guard false-positive. Edits were benign (comment-only; engine still 63/63 green) and in-spirit, but out-of-lane. Cheap-tier risk: weaker model satisfies the guard by any means, incl. reaching outside its reservation. |

## Final tally — ✅ DONE (current_state: done, dod_confirmed: true)
- **All 13 states complete.** FINAL AUDIT PASSED (66 checks, all 11 [dod.N] executed-PASS, all 7 [ref:] conform). Exit 0.
- **Two transition-engine bugs surfaced at the finish line** (skill-feedback candidates, both worked around in source):
  1. `state-transition.js` runs the GUARD from repo root (L203) but the DOD-CONFIRMATION audit from `cwd:planDir` (L114) — inconsistent. Repo-root-relative audit checks failed 4/128 in the confirmation pass. Fix: `os.chdir(repo_root)` in the audit script (made it cwd-robust). Engine should run both from the same cwd.
  2. `parseDodIds` read the inline template token `[dod.N]` in the README *prose* as a 12th clause with no PASS line → permanent `dod_unconfirmed`. Fix: reword the prose. The parser should only read DoD *bullet* clauses (gap-check already does).
  - Also earlier: the `--complete` forward-audit false-alarm on work states; and stray service orphans accumulating across harness runs (per-harness `orphans=0` holds, but the supervisor keeps installed services alive across runs → port contention).
- Executor dispatches: **10** (8 states 1-shot on sonnet + tg-service in 2 [529+fix] + 1 code-review-gate fix + 1 haiku decouple). Opus never used.
- Total `subagent_tokens`: **~795k** (8 states ~601k + tg-service fix 27k + tg-cli was counted + decouple 46k haiku + code-review fix 43k). Final ≈ **0.8M** — dead-on the low end of the original 0.5–1.5M recalibration, ~10x under the wrong-unit 4–8M first guess.
- **Guards earned their keep 3×:** the hollow tg-service build (ERR_HTTP_HEADERS_SENT), the cross-project lint/test gaps (code-review), and a [ref:] conformance leak (host-keyed-target). None could pass as "done".
- **Recurring guard-precision finding (3×):** negative greps (`red.?team`, `.claude`) false-positive on comments *asserting absence*. Fixed in source each time (reword the comment), but the audit greps should exclude absence-assertion comments — skill/plan feedback candidate.
- **Audits free when clean:** core, framework, service phases cost ~0; only final needed one source fix.

## Orchestrator lesson (tg-service attempt 1)
The orchestrator (me) twice read tg-service as "almost certainly complete" from **build-green + the demo/out artifacts existing** — but the artifacts recorded a FAILED round-trip (`tokenguard: proxy error`). **Build success and artifact existence are NOT completion signals; the GUARD is the only authority.** Correct protocol: run `--complete` (which runs the guard) or the guard directly BEFORE claiming a state is done — never infer from the build or from the presence of output files. The guard did its job here (caught a runtime bug a green build hid); the miss was the orchestrator narrating ahead of it. The user caught it ("it never ran") before the loop did.

## Transition-engine note (v0.8.13 — skill-feedback candidate)
`state-transition.js --complete core-engine` returned `status: audit_failed (0/20)` **even though the state completed correctly** (state.json `core-engine: complete`, `end_ref` set, `nx build` green, `core-engine.1–5` all PASS). The 0/20 is a *forward-looking* run of the **framework**-phase audit (20 criteria) after `current_state` advanced — it can't pass mid-plan. A `--complete` should report on the **completed** state's guard/criteria, not a look-ahead phase; the misleading `audit_failed` could make an orchestrator falsely halt. → file as a fix for plan-state-machine.

## Running totals
- Execution dispatches recorded: **6 complete** (5 1-shot + tg-service in 2: 529-interrupt + 27k fix) + audit-core/framework (free)
- Execution `subagent_tokens` so far: **~601,676** (454,567 + ~120,000* tg-service attempt-1 estimate + 27,109 fix). *attempt-1 truncated by 529; estimate from 63 tool-uses.
- States complete: **8 of 13** (`tg-service` ✅ — LIVE round-trip proven; core + framework + service-proxy all done)
- **Streak note:** "all 1-shot" broke at tg-service — but not on logic: a 529 infra blip + a single Node header-ordering runtime bug the green build couldn't catch. The fix was 27k tokens / 1 pass on sonnet. Escalation to opus STILL never needed across the whole run.
- **TIERING RESULT (settled):** every "opus"/V.Hard-rated state (core-engine, http-transport, mcp-as-service) passed **1-shot on sonnet**. The escalation ladder never fired across 5 dispatches. Conclusion: the difficulty/model ratings systematically over-estimated; **sonnet is the right default tier for this whole plan**, and per-task opus routing would have been wasted spend.
- **Re-projected total (n=5):** done 455k + 3 remaining work states (tg-service ~140k, tg-cli ~80k, decouple ~30k) + code-review ~70k + audits (cheap if clean) ≈ **~0.8M subagent_tokens**. Holding the low end of the original 0.5–1.5M band.
- **Model-tiering note:** all dispatches ran on `typescript-pro`'s default **sonnet** (no override). core-engine + service-type's "opus"-rated tasks PASSED on sonnet → ratings look too conservative. Policy going forward (R6 escalation ladder): default sonnet, escalate to opus only on repeated guard failure; route the all-haiku `decouple-generalize` state to haiku.
- **Re-projected full plan (n=2):** done 147k + 7 remaining work states (~700k tiered) + audit fix-loops (~140k) ≈ **~1.0M subagent_tokens** (range 0.8–1.6M). Converging on the prior ~0.7M recalibration; both 1-shot passes keep it at the low end. Greenfield port (53k) < live-edit (94k) — editing live code with a no-regress test suite costs ~1.8× a clean port, as expected.

## Recalibration log

**After #1 (core-engine):** 52,751 — **6–10× under** my 300–500k guess for this state.
Two things were wrong in the original estimate:
1. **Metric mismatch.** `subagent_tokens` (what I can measure) ≠ the loose "combined
   input+output throughput" I estimated in. The original 4–8M figure was in the wrong,
   broader unit. **Recalibrating in `subagent_tokens` from here.**
2. **Overestimate even so.** core-engine is a Hard state (porting a 27KB engine) and
   still came in at ~53k because it ran *clean* — no audit-fix loop, no build failures.

**Re-projected full plan (in `subagent_tokens`, executor dispatches only):**

| Tier | States | Est. each | Subtotal |
|---|---|---|---|
| Easy | decouple-generalize | ~25k | 25k |
| Med | service-type, core-invariants, tg-cli, code-review | ~50k | 200k |
| Hard | http-transport, tg-service | ~90k | 180k |
| V.Hard / debug-loop risk | mcp-as-service | ~120k | 120k |
| Hard (actual) | core-engine | 52,751 | 53k |
| Audits ×4 | clean→cheap, fix-loops→expensive | ~35k | 140k |
| **Executor total** | | | **~720k** (range 0.5M–1.5M) |

- **Orchestrator (my own tokens)** are separate and not in `subagent_tokens`; they accumulate across the loop.
- **Dominant variance = failure loops.** core-engine ran clean; if `mcp-as-service` (C6 non-regression) or `tg-service` (streaming) hit repeated build/audit failures, those states alone could 2–4× their estimate. The risk is asymmetric (upside).
- **Net:** the realistic executor-side total is **~0.7M `subagent_tokens` (range 0.5–1.5M)** — an order of magnitude below the original 4–8M, which was both the wrong metric and too high. Next datapoint (`service-type`) tests whether a *framework-edit* state (vs. a greenfield port) holds the ~50k figure.
