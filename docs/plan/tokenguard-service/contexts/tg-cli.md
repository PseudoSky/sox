...
- **Delta Spec:**
  - `src/mapstore.ts` — the single read/write owner of **[shape:token-map]** on disk, shared by the proxy and the CLI: atomic append of a `custom`/`tooling` entry, full read for inspection, and a change signal (file-watch or a memoryd-style socket nudge) the running proxy subscribes to so a new seed is applied to live traffic without restart (**[def:live-map]**).
...
- [ ] **[tg-cli.3]** the running proxy reloads on a seed change (watch or nudge). `grep -n "watch\|nudge\|reload\|socket" extensions/services/tokenguard/src/mapstore.ts`
