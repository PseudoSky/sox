...
After this state, `service` is a **declarable manifest type** carrying a `transports` array (`stdio|http|sse|socket`), and `sox init service <id>` scaffolds a **born-conformant** service extension that builds and validates with zero manual edits. This promotes the existing "service is an execution mode" comment into a first-class primitive — the foundation the http transport ([http-transport]) and the mcp-server fold-in ([mcp-as-service]) both build on. It does **not** yet route http or refactor mcp-server; it only makes the type real and scaffoldable.
...
- **Added:** `service` type + `transports` field (manifest); `serviceTemplate` (authoring); `docs/guidelines/service.md`; `tools/tg-plan/check-service-scaffold.sh`; `sox init service` routing in `apps/sox/src/main.ts`.
...
- `dist/index.js` in the template must be a runnable stub that errors if executed but lets `sox validate` pass entrypoint-reachability immediately (mirror `mcpServerTemplate`).
