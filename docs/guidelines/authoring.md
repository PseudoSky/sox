...
**Build/publish** — declarative agents have no `dist/`. Registering a checksum row in
`registry/index.json` is a release-flow step, not a local one — see
[AGENTS.md § registry is release-only](../../AGENTS.md#registry-is-release-only). Locally, commit
`extension.json` + `agent.md`; the extension installs from its local dir with no checksum gate
until it has a released row.
...
**A local change to a code extension's artifact never regenerates the registry** — see
[AGENTS.md § registry is release-only](../../AGENTS.md#registry-is-release-only). An extension
with no registry row installs from its local dir with no checksum gate.

Never hand-edit `registry/index.json`. `source` is a checkout-bound `file://` path in local
development; at publish time, with `SOX_REGISTRY_PUBLISH=npm`, it is rewritten to a portable
`npm-package:` locator (see [PUBLISHING.md](../../PUBLISHING.md)).
...
Build the **code members** (declarative members need nothing), validate, install the whole set —
no registry step, see [AGENTS.md § registry is release-only](../../AGENTS.md#registry-is-release-only):
...
| `CHECKSUM MISMATCH` on install | A local extension with no registry row has no checksum gate; a pinned row mismatch means the local build is not the published bytes — install from local or repin via `tools/repin-registry-entry.mjs`. |
...
- ADRs: [0003 content-address](../decisions/0003-extension-identity-is-content-addressed.md) ·
  ...
  [0021 committed registry is publish-shape](../decisions/0021-committed-registry-is-publish-shape.md).
