...
Citations: packages/apigen/apigen-plugin-ir-cache/vite.config.ts (pre-fix state at commit 79aa19f5^), packages/apigen/apigen-plugin-batch/project.json, tools/nx-plugins/assets/executors/copy/impl.js:28, tools/nx-plugins/build/plugin.js (retired-link + in-source-dist comment).
