...
**Version:** 6 — Consolidated 17 gap fixes from full meta-loop audit. Phase C trace
check moved before examples. Empty trace guidance added. Step numbering fixed.
General-purpose subagent mandated for dispatch. Self-consistency check proceduralized.
...
If the Meta-cognitive trace is empty (first iteration), skip the trace check.
...
**Proceed gate:**
If all phases A-F are complete and the RQs are researchable externally, proceed to
Application Mode. If uncertain about any phase, restart from Pre-Commitment.
...
**Stopping criterion:** If Loop 2 identifies zero process failures AND all findings pass the confidence anchors, the process is stable. Stop and log completion. Otherwise, proceed to Loop 3.
...
Read the complete artifact file from Step 1. Dispatch a general-purpose subagent
(not a domain-specific agent — use the most flexible available agent type) with
ALL of the following inlined in their prompt:

```
CWD: <the current working directory path from the editing agent's environment>
...
Iteration Manifest, PLUS any structural issues encountered during execution
(contradictions, missing sections, ambiguous instructions). The report must
include evidence that every phase was completed (Position Declaration records,
phase outputs, etc.).
...
5. In Self-Analysis mode: a self-consistency check has passed — the modified artifact
   ...
   context. Procedure: have a fresh agent read the artifact and verify they can
   identify their starting loop and first action without asking questions.
