...
**Version:** 5 (proposed) — Adds meta-cognitive trace for cross-iteration pattern accumulation. Each iteration now references and updates a persistent trace of recurring tensions, heuristics, and failure modes.

**Changelog:**
- v5 (proposed): Added Meta-cognitive trace field to Iteration Manifest. Pre-Commitment references prior traces. Loop 2 self-feedback updates the trace.
- v4: Added Loop 3a (real execution test) + Loop 3b (conditional commit). Loop 3 → propose-only. Loop 4 → structural verification only. Requires exact measurement procedures in dispatch payload.
- v3: Added scope boundary on Self-Refine (same-agent vs dispatched). Added mandatory subagent dispatch for Loop 4.
...
articulate completion concretely rather than skipping. Example evidence:
...
After identifying the tension, check the Meta-cognitive trace: has this tension
...
**Stopping criterion:** If Loop 2 identifies zero process failures AND all findings pass the confidence anchors, the process is stable. You may stop. Otherwise, proceed to Loop 3.
...
Output: a list of proposed section replacements, each clearly marked as PROPOSED.
...
1. **Collect all proposed section replacements** from Loop 3 into a single coherent
   proposed version of the affected portion of the iteration target.

2. **Construct the complete proposed artifact** — the FULL iteration target as it
   would look if all proposed changes from Loop 3 were applied. This is NOT a diff,
   NOT a list of section replacements. It is the complete, contiguous, executable
   artifact. The subagent must never open the original file.

3. **Dispatch a subagent with EVERYTHING inlined in their prompt:**
   - The COMPLETE proposed artifact (every section, every word) inlined as text.
     The subagent receives NO file path, NO reference to the original file.
     They execute ONLY what is in their prompt.
   - ALL baseline metrics from the Iteration Manifest, EACH with:
     - The exact step-by-step measurement procedure (unit of analysis, detection
       rule, exclusion criteria, counting method, measurement point in the flow)
     - The baseline value
   - Instructions to execute the process using ONLY the inlined text — no external
     files, no lookups, no references to "the current version."
...
4. **The subagent MUST execute the proposed process** as if it were the real
   process — search, analyze, produce output, record findings. This is NOT a
   review or a simulation — it is a real run using the proposed instructions.
   If the proposed process has multiple phases, the subagent completes all of them.

5. **The subagent reports:** the measured value for each baseline metric.
   No interpretation, no recommendations — just the numbers and the raw output
   that produced them.

6. **The editing agent computes deltas**: `proposed_result − baseline` for every
   metric. Compare each delta against the success criterion threshold from the
   Iteration Manifest.

7. **Decision:**
   - **All deltas meet or exceed thresholds** → proceed to Loop 3b (commit).
     Record the real deltas in the Iteration Manifest's Measured delta field.
   - **Any delta falls below threshold** → do NOT commit. Return to Loop 2.
     The subagent's raw output is diagnostic evidence for the next analysis cycle.
