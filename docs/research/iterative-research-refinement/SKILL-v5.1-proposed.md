...
### Step 4 — Subagent executes the ENTIRE applicable portion of the process

The subagent MUST execute the proposed process from Position Declaration through
all applicable phases that do not require editor-level authority. Specifically:

- **Execute**: Pre-Commitment, Observation Generalization (A-F), Loop 1 (Phases 0-5),
  Loop 2 (Process Audit). These are the research and analysis phases.
- **Do NOT execute**: Loop 3 (Propose Changes), Loop 3a (Real Execution Test),
  Loop 3b (Commit Changes), Loop 4 (Structural Verification). These are the
  editing agent's responsibility. The subagent cannot dispatch itself.
- For any section that says "dispatch a subagent" or "edit the file", the subagent
  skips that instruction and records in their report: "Skipped [section] — this
  is editing-agent responsibility per the process scope boundary."
