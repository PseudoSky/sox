...
**Version:** 6 — Structural fixes: Confidence Anchors moved to top, self-contradictions resolved,
ambiguous instructions clarified, baseline procedure added, Self-Analysis mode specified,
RFC 2119 keywords adopted, instruction/example separation enforced.

**Changelog:**
- v6: Moved Confidence Anchors before first reference. Fixed Phase 5 "before reading"
  contradiction (moved to Phase 3). Removed threshold-adjustment contradiction in
  Re-Measurement. Clarified "you may stop" = exit entire process. Added concrete
  Self-Analysis mode procedure. Added evidence gate between Application Mode and Loop 1.
  Added baseline measurement procedure. Added Phase 5 audit record location. Added
  instruction/example separation (blockquotes). Added instruction-sequencing and
  cross-section-consistency to Loop 2 failure classifications. Moved "Pass" definition
  into Loop 2 stopping criterion. Added trace pruning guidance. Adopted RFC 2119
  keywords (MUST/SHOULD/MAY) consistently.
...
- v3: Added scope boundary on Self-Refine (same-agent vs dispatched).
  Added mandatory subagent dispatch for Loop 4.
...
refinement (improving a single response within one context window). Same-agent evaluation
exhibits self-preference bias — the agent favors its own outputs over external evaluation.
...
structurally incapable of detecting its own blind spots.

---

## Confidence Anchors

These anchors apply to EVERY claim made during research. You MUST use them in all
findings, self-feedback, and audit records.
...
If confidence is LOW, you MUST state explicitly: "I believe this, but I cannot verify it from my research."

**"Pass" definition for confidence anchors:**
...
- If any LOW-confidence finding exists after Loop 2, record it in "Known gaps from prior work"
  and proceed to Loop 3 — do NOT stop even if no process failures were identified.

---

## Before Starting: Position Declaration

Before ANY action, you MUST state your current Loop and Phase. This is a runtime guard against skipping.
...
If Previous Phase is No, you MUST STOP and complete it before proceeding.
...
> **Example evidence:**
> "Completed Phase 3: deep-read Self-Refine paper (A-grade), recorded CRAAP
...
Before beginning any iteration, you MUST declare what you are iterating on and what this
...
- Metric 1 — <name>: <value>   (e.g., "Structural gaps found by fresh subagent: 14")
- Metric 2 — <name>: <value>   (e.g., "Phases with execution issues: 3")
...
> **Trace pruning guidance:**
> - Maximum 10 patterns. When adding an 11th, merge the two most similar patterns or
>   retire the oldest pattern that has not appeared in the last 2 iterations.
> - A pattern discovered during a failed iteration SHOULD still be recorded if it is
>   independently observed (not an artifact of the failure).
...
- [ ] <metric>: <baseline_value> → <target_value>, measured by <method>
      Example: "Structural gaps: 14 (baseline subagent) → ≤3 (v6 subagent), measured by dispatched subagent report"
- [ ] <metric>: <baseline_value> → <target_value>, measured by <method>
...
you MUST establish a quantified baseline for each success criterion metric.

### How to take baseline measurements

You MUST follow this procedure before filling in the Iteration Manifest's
Baseline measurement field:

**Step 1 — Identify what to measure.**
Each success criterion metric needs a concrete measurement method.
Metrics MUST be countable (numbers, not subjective assessments).

**Step 2 — For structural metrics (contradictions, missing gates, coverage):**
Scan the iteration target yourself. Count each instance. Record the raw counts.

**Step 3 — For execution metrics (gaps found by fresh agent, phase issues):**
Dispatch a fresh subagent with the CURRENT artifact (not proposed changes) inlined
in their prompt, along with a real research task. The subagent MUST execute
the complete process from Position Declaration through Loop 2. Count the issues
they report.

**Step 4 — Record all baselines** in the Iteration Manifest's Baseline measurement
field. You MAY use placeholder values ("not yet measured") for metrics that require
a subagent dispatch, but you MUST replace them with actual values before the
iteration exits Loop 2.

### Baseline rules
- Each metric must be a concrete, countable value (not subjective "seems fine")
- You MUST take the measurement BEFORE editing the iteration target
...
- "Number of structural gaps the current artifact would cause a fresh agent" —
  measured by dispatching a fresh subagent to execute the artifact before changes
...
- "Confidence anchor violations" — measured by scanning the artifact
  for claims not tagged with a confidence level

### INVALID baseline metrics (rejected)
...
Before any search, you MUST declare:

**Priors — what I currently believe about this topic:**
[State your existing assumptions. Be specific. For each prior, note its provenance:
where did I learn this? If from the process document itself, mark as "process-internal"
and flag for verification during Loop 1.]
...
If none apply, state "None — no prior traces match this iteration's topic."]
...
After identifying the tension, you MUST check the Meta-cognitive trace: has this tension
appeared before? If yes, reference the existing pattern. If this tension contradicts
a prior trace, flag it for trace revision — do not create a duplicate entry.
...
Before proceeding to Loop 1, you MUST determine the mode of this iteration:
...
- **Self-Analysis mode**: The subject IS this skill file (or another process
  ...
  topic. In this mode, Loop 1 Phases 0-4 serve a SUPPORTING role — the PRIMARY
...
### If Self-Analysis mode

Loop 1 procedure is modified as follows:

**Phase 0 (Search):** Search for process document design frameworks that the artifact
can be compared against. Examples: RFC 2119 (normative language for technical
specifications), structured prompting patterns, checklist design principles.

**Phases 1-3 (Click, Triage, Deep Read):** Select and read sources on executable
document design, agent SOP patterns, and structured instruction formats. These
provide external reference points for your structural analysis.

**Phases 4-5 (Trace Claims, Self-Feedback):** Apply the framework principles from
your external sources to the artifact. Does the artifact follow the patterns these
sources recommend? Record structural gaps as your primary findings.

**Primary analysis:** You MUST ALSO perform a structural scan of the artifact itself:
scan every section for contradictions, forward references, missing gates, ambiguous
control flow, and mode-specific gaps. These structural findings are your PRIMARY
evidence. The external sources provide supporting calibration.

### Proceed gate

After recording your mode and (if Self-Analysis) confirming you understand the
modified Loop 1 procedure, record:

```
Application mode: <Self-Analysis / External Research>
Structural scan scope: <sections to analyze, or "N/A" for External Research>
Proceed to Loop 1? Yes/No
```

If No, you MUST resolve the blocker before proceeding.
...
You MUST generate self-feedback BEFORE deep-reading each source: "What do I expect this source to say? What would confirm or contradict my priors from Pre-Commitment?"
...
**Self-feedback (after reading):** Compare your Phase 3 expectations against what the source actually said. Were your priors confirmed or contradicted?

**Audit record — you MUST record this in your response output, not in a separate file:**
- Search query used
...
- **Confidence** (use the anchors defined at the top of this document)
...
3. **Classify each process failure** using one of these categories:
   ...
   - **Instruction sequencing** — temporal qualifier contradicts document execution order (e.g., "before reading" in a phase that occurs after reading)
   - **Cross-section consistency** — two sections make contradictory claims about the same action
   - **Control flow precision** — branching instruction doesn't specify scope (e.g., "you may stop" without defining what "stop" means)
   - **Modal completeness** — a mode description lacks specific operational steps
   - **Declarative gap** — describes desired output but not the procedure to produce it
   - **Output specification** — requires an output but doesn't specify where to record it
4. **Self-feedback**: What pattern in YOUR OWN execution produced this error? Is this a recurring pattern? If you identify a recurring pattern, record it as a new entry in the Meta-cognitive trace.
   > **Example trace entry:**
   > "Pattern: every time I search with broad terms, I get irrelevant results.
   > Heuristic: use the framework's own vocabulary."
5. **Patch the process**: For each failure, write a rule that prevents recurrence.

### Stopping criterion

- If Loop 2 identifies zero process failures AND all findings pass the confidence anchors (zero LOW-confidence findings remain), the process is stable. **Stop = exit the entire process. Your research findings are final. Do NOT proceed to Loop 3.**
- If Loop 2 identifies process failures, proceed to Loop 3.
- If any LOW-confidence finding exists (even with zero process failures), record it in "Known gaps from prior work" and proceed to Loop 3. Do NOT stop.

---

## Loop 3 — Propose Changes (do NOT edit the file)

Render the patched process as a proposal. Do NOT apply changes to the iteration
target file yet — the proposal must be tested by real execution first.
...
Output:
- A list of proposed section replacements, each clearly marked as PROPOSED
- The COMPLETE reconstructed artifact written to a new versioned file (e.g.,
  `SKILL-v5-proposed.md`). This file is the input to Loop 3a Step 1. Construct
  it by applying all proposed section replacements to the current iteration target.
...
Loop 3 already wrote the complete proposed artifact to a new versioned file.
Use that file as the record of what will be tested. Never edit the original file.

The file path is the one produced by Loop 3's output. Verify it exists before
proceeding. If it does not exist, return to Loop 3 — the file must be created
before the execution test can proceed.

### Step 2 — Generate a full real-world example task

Create a concrete task that exercises the ENTIRE process from Position
Declaration through to the final iteration exit criterion. The task MUST:
...
Read the complete artifact file from Step 1. Dispatch a subagent with ALL of
the following inlined in their prompt:
...
including sections that reference editor-level actions. Specifically:

- **Execute fully**: Pre-Commitment, Observation Generalization (A-F),
  Loop 1 (Phases 0-5), Loop 2 (Process Audit). These are the research and
  analysis phases that the subagent performs in full.
- **Read and process, but simulate instead of executing**: Loop 3 (Propose
  Changes), Loop 3a (Real Execution Test), Loop 3b (Commit Changes),
  Loop 4 (Structural Verification), and any "dispatch a subagent" or
  "edit the file" instructions. For each such instruction, the subagent
  records in their report: "Would dispatch [description of what would be sent]"
  or "Would edit [file path] with [summary of changes]."
- The subagent MUST still read these sections and evaluate whether they are
  internally consistent, reference real sections, and could be followed by
  the editing agent. Any contradictions or errors found in these sections
  are reported as part of the test results.
...
The subagent reports: the measured value for each baseline metric from the
Iteration Manifest. No interpretation, no recommendations — just the numbers
and the raw output that produced them. The report MUST include evidence that
every phase was completed (Position Declaration records, phase outputs, etc.).
...
- **All deltas meet or exceed thresholds** and the subagent completed all phases
  ...
  Manifest's Measured delta field.
- **Any delta falls below threshold**, OR the subagent did not complete all
  ...
  Return to Loop 2. The subagent's raw output is diagnostic evidence.
...
- The Loop 3a execution test already prevents this — if it didn't, the gap is
  in the threshold definition, not the measurement. Do NOT adjust thresholds
  retroactively (this would violate Promotion Gate rule 4).
