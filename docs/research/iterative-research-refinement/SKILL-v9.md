...
variant before promotion. Measurement is deterministic via the bundled script
`scripts/runtime-metrics.mjs` (or the opencode custom tool `runtime-metrics`). A
variant that regresses runtime beyond threshold does NOT promote unchanged.
Proposed artifact path is deterministic: `<CWD>/<basename>-v<N+1>.<ext>`.
Subagent return messages capture vN+2 results.
CWD field stays in the Iteration Manifest.

**Changelog:**
- v9: Runtime metrics made mandatory (process failure 2026-08-11: debug v1→v2 promoted
  on quality alone while using 1.4–1.9x tool calls, 1.4–2.1x wall-clock). Iteration
  Manifest gains a "Runtime metrics" block. Loop 3a dispatch payload MUST include the
  runtime measurement command. Promotion gate adds a runtime-delta criterion.
  Bundled script `scripts/runtime-metrics.mjs` provides the deterministic method.
- v8: Added CWD to Iteration Manifest. Loop 3/Loop 3a now use deterministic output
  path formula. Loop 3a Step 5 requires vN+2 subagent results in return message.
...
Runtime metrics (MANDATORY for any iteration with a real-execution or A/B test —
see §Runtime Metrics below):
- Tool calls (baseline): <n>
- Wall-clock seconds (baseline): <n>
- Cost $ (baseline): <n>
- Input tokens (baseline): <n>
- Output tokens (baseline): <n>
- Reasoning tokens (baseline): <n>
- Cache reads (baseline): <n>
- Messages (baseline): <n>
**Measurement method:** `node ~/.config/opencode/skills/iterative-research-refinement/scripts/runtime-metrics.mjs <ses_id> [...]`
captured from the task tool result after every dispatch. Record each run's session id.
(Script paths are relative to the skill dir, NOT the project CWD — see §Runtime Metrics.)
...
6. **Runtime metrics were measured and reported for baseline AND variant** (when the
   iteration involves any real-execution/A-B test — i.e. almost always), and each
   runtime delta is within threshold (defaults: tool calls ≤1.25x, wall-clock ≤1.25x,
   cost ≤1.15x baseline) OR the cost-for-quality tradeoff was explicitly surfaced to
   the user for a decision. "No metric regressed" may never be claimed without the
   runtime table.
...
## Runtime Metrics (MANDATORY for real-execution / A-B iterations)

Quality metrics alone are insufficient for promotion. A variant can pass every
quality/policy criterion while silently regressing cost (proven 2026-08-11: debug
agent v1→v2 passed quality on 2 real bugs yet used 1.4–1.9x tool calls, 1.4–2.1x
wall-clock, cost +9%/+61%; measured from the opencode session DB). Every iteration
that involves dispatching real-execution runs (Loop 3a) MUST capture runtime
telemetry for baseline AND variant and report it as part of the measured delta.
Research-only iterations (Loops 1–2 with no subagent dispatch) do NOT need a runtime
table — but if ANY dispatch happens, this section applies to that dispatch.

**Skill directory (anchor for all script paths):**
`~/.config/opencode/skills/iterative-research-refinement/` — all `scripts/...`
paths below are relative to THIS directory, not to the project CWD.

**Prerequisites (verified 2026-08-11):** `sqlite3` on PATH; the opencode session DB
at `~/.local/share/opencode/opencode.db` containing `session` (cost, tokens_*,
time_created/time_updated) and `part` (tool-call count via `json_extract(data,'$.type')='tool'`).

Metrics (per run): tool calls, wall-clock seconds, cost $, input tokens, output
tokens, reasoning tokens, cache reads, messages.

Measurement method (deterministic, bundled with this skill):
```
node ~/.config/opencode/skills/iterative-research-refinement/scripts/runtime-metrics.mjs <ses_id> [...] \
  --label <ses_id>:<label> ... --pairs <baseline_id>:<variant_id> ...
```
- The script reads the opencode session DB and emits the table + threshold verdicts.
- **Every dispatched run returns a session id** in the task tool result; record it
...
- cost ≤ 1.15x baseline
- input tokens should NOT rise when the variant has a shorter prompt (report either way)

The script auto-flags tool/time/cost violations. **The input-token direction check is
enforced by the EDITOR, not the script** — compare the reported input_tok values by
eye and note any rise explicitly in the Measured delta record.

Decision rule — quality vs cost:
- If the variant beats baseline on quality/policy AND all runtime deltas are within
  threshold → promote.
- If the variant beats baseline on quality/policy BUT regresses runtime beyond
  threshold → do NOT promote unchanged. Either (a) re-iterate to bound the
  verification loop (e.g. "one reproduction run, then report" instead of exhaustive
  empirical verification), or (b) explicitly surface the cost-for-quality tradeoff
  to the user for a decision. Never silently accept a runtime regression.
- If the variant regresses quality → do not promote, regardless of runtime savings.
...
3. **Classify each process failure**:
   ...
   - **C-grade numeric lift** — lifted a number from a B/C-grade blog without tracing
     to the A/B primary (rule: trace or mark LOW; see Confidence Anchors)
...
### Step 5.5 — Editor captures runtime metrics (MANDATORY)

Immediately after the subagent returns, the EDITOR records the subagent's session id
(from the task tool result) and runs the runtime measurement:

```
node ~/.config/opencode/skills/iterative-research-refinement/scripts/runtime-metrics.mjs <baseline_ses_id> <variant_ses_id> \
  --label <baseline_ses_id>:baseline --label <variant_ses_id>:variant \
  --pairs <baseline_ses_id>:<variant_ses_id>
```

(Path is relative to the skill dir, NOT the project CWD — see §Runtime Metrics for
the anchor, prerequisites, and the input-token editor check.)

Append the resulting runtime table + verdicts to the iteration record (the Iteration
Manifest's Runtime metrics block). This happens for EVERY dispatched run, baseline and
variant — do not skip the baseline side. Without the runtime table, Step 7's promotion
decision is incomplete.

### Step 6 — Editor computes deltas

The editing agent computes `proposed_result − baseline` for every metric
(quality metrics AND runtime metrics from Step 5.5).
Compare each delta against the success criterion threshold from the
Iteration Manifest and against the runtime thresholds (§Runtime Metrics).
...
- **All deltas meet or exceed thresholds** (quality AND runtime — see §Runtime
  Metrics decision rule), and the subagent completed all phases
...
- **Runtime regression beyond threshold with quality gain** → do NOT commit
  unchanged: either bound the verification loop and re-iterate, or surface the
  cost-for-quality tradeoff to the user for a decision.
...
2. **Include the runtime deltas from Loop 3a Step 5.5** (tool calls, wall-clock, cost,
   tokens, cache reads) in the Measured delta record — quality deltas alone are not
   a complete measurement.
3. Record each delta in the Iteration Manifest's "Measured delta" field
4. Verify each delta against the corresponding success criterion threshold AND the
   runtime thresholds (§Runtime Metrics)
...
in each success criterion. Runtime measurement uses the same bundled script
(`scripts/runtime-metrics.mjs`) for both sides — never a hand-typed estimate.
...
3. The promotion gate (all success criteria met, no retroactive weakening) has passed,
   **including the runtime-metrics criterion (promotion gate item 6) when the iteration
   involved real-execution testing.**
...
Lateral-triage grading note (2026-08-11 execution-test finding): first-party builder
engineering posts (Anthropic "How we built…", Cognition "Don't build multi-agents")
are **B-primary** — stronger than generic B (they are the primary source for their own
system's behavior) but weaker than peer review. Do not force them into A; cite as
B-primary. **Numeric claims lifted from C-grade blogs never enter findings as-is** —
trace to the A/B primary or mark LOW.
