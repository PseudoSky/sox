...
| S8 | Clustering actually runs (BL-349/BL-326) | **→ research done, REDESIGN required** | PKT-28 (`findings/pkt28-clustering-strategy.md`) measured at the TRUE full corpus (N=4867, not a 1616 sample): a fixed global τ is **not viable at any value** — τ=0.82 gives largest-cluster ratio **0.759** (was 0.684 four days ago; density worsens as the store grows), 0.85 has now crossed into degenerate (0.514), only 0.87 holds (0.181). "Raise minPts" **directly ruled out**: 6× minPts moves 0.759→0.6365 while clustered_frac collapses 0.886→0.659 — it is a purity filter, not a percolation fix. Replacement: target-mean-degree calibration at cluster time. BL-350 RESOLVED with a working drift harness (measured **72.8% drift**, 0 live assignments vs 3544 a full pass would make). BL-356/BL-328 stay open — their acceptance needs a code-level red→green, which is PKT-30's job. |
