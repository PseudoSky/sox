...
Status of every BL item lives in the backlog graph: query it directly (`backlog query` /
`backlog get`, or the `backlog_query`/`backlog_get` MCP tools) — this file carries no
machine-derived ledger or packet-completion summary. The per-packet ledger used to live in
[`PLAN.md`](./PLAN.md); PLAN.md now points at the same graph query instead.
...
> ⚠️ **This section is hand-written and goes stale silently — no guard catches it.** Before acting
> on an item here, confirm it against the backlog graph directly (`backlog query` / `backlog get`,
> or `backlog_query`/`backlog_get`, repo `sox-ecosystem`) — not against this prose. As of 2026-08-04
> this list led with **PKT-41 (BL-391) + PKT-19 (BL-329)**, both of which had already been done for
> some time. A session acted on that entry before catching it.
...
4. **The backlog graph is the sole source for `BL-*` items — no reconciliation left to do.**
   ...
   were migrated and existence-verified per id. Query the graph directly (`backlog query` /
   `backlog get`) for status; `tools/check-backlog-markers.mjs`/`tools/check-bl-id-integrity.mjs`/
   `tools/allocate-bl-id.mjs`/`tools/plan-status.mjs` are all retired — the first two now refuse to
   run and point at the graph, and the latter two are deleted outright. There is no markdown write
   surface left for this family; file everything through `backlog_create_item`.
