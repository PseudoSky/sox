...
>
> **How to use this file.** Status of every `BL-*` item lives in the backlog graph — query it
> directly (`backlog query` / `backlog get`, or the `backlog_query`/`backlog_get` MCP tools) for
> what is actually done; this file carries no machine-derived ledger or per-packet status line.
> Prose in this file is **design rationale**, written at various dates, and may describe a defect
> that has since been fixed. Sections marked **COMPLETE** are kept for the reasoning, not as work.
...
> **RECONCILED 2026-08-03 — the coverage gap this warning described was closed at the time.** It
> previously read that 53 open items (62%) appeared nowhere in the plan. Every open in-scope item
> had a packet as of that date. Query the backlog graph directly (`backlog query` / `backlog get`)
> for current coverage — this file carries no machine-derived ledger.
>
...
> **This list is enforced, not decorative.** Cross-check it against the backlog graph directly
> (`backlog query` for open in-scope items with no packet) rather than trusting this section alone
> — dropping an item from the plan should be verifiable against the live graph, not just this file.
>
