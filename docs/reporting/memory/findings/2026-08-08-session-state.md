...
| `feat/cluster-tracing` | guard's instrument — 5-outcome taxonomy, `stages.ts` integration, lifecycle counters, backlog off `edge` rows; reframed at `453058c7` as the acceptance criterion | green (memory-core 633 passed, memory-server 230, tree CLEAN); **conflicts with BL-496 on 7 files**; does not merge mechanically |
