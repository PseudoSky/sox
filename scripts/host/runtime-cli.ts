/**
 * scripts/host/runtime-cli.ts — CLI entry point for sox start / sox stop.
 *
 * Invoked by bin/sox via: npx tsx scripts/host/runtime-cli.ts <verb> [options]
 *
 * Verbs:
...
async function cmdStart(flags: Record<string, string>): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(`sox: runtime supervisor running (pid=${process.pid}) — Ctrl-C or 'sox stop' to stop\n`);
    ...
    // Keep alive
    setInterval(() => {
      // Heartbeat: update running state in the record
      const currentRecord = getRuntimeRecord(runtimeFilePath);
      if (currentRecord) {
        // No-op heartbeat — the supervisor manages process state internally
      }
    }, 5000);
...
async function cmdStop(flags: Record<string, string>): Promise<void> {
  ...
  if (runningEntries.length === 0) {
    ...
    if (!id) {
      ...
      const { dirname } = await import('node:path');
      mkdirSync(dirname(runtimeFilePath), { recursive: true });
      const cleared = { ...record, entries: record.entries.map((e) => ({ ...e, running: false, pid: null })) };
      writeFileSync(runtimeFilePath, JSON.stringify(cleared, null, 2) + '\n', 'utf8');
    }
    process.exit(0);
  }
  ...
  process.stdout.write(
    `sox: stop complete — stopped ${stopped}, failed ${failed}\n`,
  );
...
// ─── Exec: call a tool on a running extension ─────────────────────────────────
 ...
 *
 * This is the "use through the activated runtime" mechanism — the caller
 * addresses the extension by its id (from the runtime record), not by path.
 *
 * Usage:
 *   runtime-cli exec --runtime-file=<path> --id=<ext-id> --tool=<tool-name> --args='<json>'
 */
async function cmdExec(flags: Record<string, string>): Promise<void> {
  ...
  // Read the runtime record to find the extension
  const record = getRuntimeRecord(runtimeFilePath);
  if (!record) {
  ...
  // Resolve the entrypoint from the source
  ...
  // Spawn the extension as a fresh MCP stdio session
