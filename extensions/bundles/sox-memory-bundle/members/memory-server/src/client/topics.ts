/**
 * Topics listing — SQL aggregate query with community enrichment.
 *
...
export const inputSchema = {
  ...
  properties: {
    ...
    project_path: {
      type: 'string',
      description: 'Filter to topics that have at least one episode from this project_path.',
    },
    search: { type: 'string', description: 'Partial topic name substring filter.' },
    sort_by: {
      type: 'string',
      enum: ['episode_count', 'avg_importance', 'last_written'],
...
/**
 * Backing for memory_topics.
 *
 * Lists topics in the store with episode counts, average importance, last
 * written timestamp, and community backing status.
 */
