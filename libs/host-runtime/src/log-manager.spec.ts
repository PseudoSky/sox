 ...
 * copytruncates an over-size active log to `<path>.<YYYYMMDD>` (BL-629: the
 * active inode is preserved so the supervisor's open fd keeps writing to the
 * same, now-truncated file), then prunes archives by count and size — matching
 * BOTH the new `.YYYYMMDD` form and the legacy `-os-<date>.<out|err>.log` form,
 * and deleting any archive over `maxBytes*4` regardless of count.
 */
...
describe('rotateOsUnitLogs — BL-620 / INV-6', () => {
  it('copytruncates an over-size active log to <path>.<YYYYMMDD> (preserves the live inode)', () => {
    ...
    // …and the rotated content landed in a compact `.<YYYYMMDD>` archive.
    const archived = fs.readdirSync(dir).find((f) => f.startsWith('doctor-tick-os.out.log.'));
    expect(archived).toMatch(/^doctor-tick-os\.out\.log\.\d{8}$/);
