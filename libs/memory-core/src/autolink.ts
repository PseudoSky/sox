...
export async function buildAutoLinks(
  ...
  if (totalEpisodes >= 2) {
    ...
    // DEBT-MEMORY-ENRICH-001: chunk the OUTER episode loop so the write lock is
    // released between chunks instead of held for the whole pairwise pass
    ...
    // single-transaction form — determinism is preserved by construction.
    // autoLinkChunkSize <= 0 → single transaction (original behaviour).
    const chunkSize = autoLinkChunkSize > 0 ? autoLinkChunkSize : sortedEps.length;
    for (let start = 0; start < sortedEps.length; start += chunkSize) {
      const end = Math.min(start + chunkSize, sortedEps.length);
