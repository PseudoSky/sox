...
export function serviceTemplate(opts: TemplateOpts): FileSet {
  ...
  return {
    ...
    'src/index.ts': [
      ...
      `});`,
      ``,
      `// BL-619: attach the error guard BEFORE listen — a port collision (EADDRINUSE)`,
      `// resolves as a structured "already running" exit (0), never an unhandled 'error'.`,
      `server.on('error', (err) => {`,
      `  const code = err && typeof err === 'object' && 'code' in err ? String(err.code) : undefined;`,
      `  process.stderr.write(JSON.stringify({ code, message: err && err.message ? String(err.message) : String(err), host: '127.0.0.1', port: PORT, disposition: code === 'EADDRINUSE' ? 'already-running' : 'other', pid: process.pid, ts: new Date().toISOString() }) + '\\n');`,
