/**
 * autolink.ts — RELATES_TO auto-link via shared entity co-occurrence (E9).
 * CONTRACTS.md C1.9, DESIGN.md D2 E9.
 *
 * Determinism: for a fixed DB state, always produces the same edge set.
 * No LLM, no network. Idempotent (only inserts edges that don't already exist).
 */
...
export interface AutoLinkResult {
  ...
  /** Entity UIDs that were added to the per-DB stoplist this pass (> threshold). */
...
export function buildAutoLinks(
  ...
  // Count total live episodes (for stoplist threshold computation)
  const totalRow = db
    .prepare<[], { cnt: number }>(
  ...
  if (totalEpisodes < 2) return { edges_inserted: 0, stoplist_additions: [] };

  // Compute entity frequency (how many episodes each entity appears in)
  ...
  // Build stoplist: entities in > threshold fraction of episodes
  ...
  // Persist stoplist to memory_scope.meta["entity_stoplist"] (best-effort; may not exist)
  try {
    const scopeRow = db
    ...
    // memory_scope.meta column may not exist in older stores — skip silently.
  }

  // Fetch all live episode→entity MENTIONS edges (excluding stoplist entities)
  ...
  // Find episode pairs sharing >= 2 entities
  // Sort episodes by rowid for deterministic traversal (ascending, per D1.2)
  const sortedEps = Array.from(episodeEntities.entries()).sort(([a], [b]) => a - b);

  // Per-episode edge cap: track how many RELATES_TO edges each episode already has
  const edgeCountPerEpisode = new Map<number, number>();

  let edgesInserted = 0;

  // Insert edges inside a transaction for atomicity + performance
  ...
  const tx = db.transaction(() => {
    for (let i = 0; i < sortedEps.length; i++) {
      ...
      for (let j = i + 1; j < sortedEps.length; j++) {
        ...
        // Per-episode cap: max 10 RELATES_TO edges
        ...
        // Insert edge (idempotent — WHERE NOT EXISTS guard)
