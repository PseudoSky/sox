// ── Pure ingest core — CJS-SAFE ENTRY POINT ─────────────────────────────────
//
// [BL-231] This module is the CommonJS-safe surface of `@adhd/sox-ingest`,
// published as the `@adhd/sox-ingest/core` subpath export.
//
// WHY THIS FILE EXISTS
// --------------------
// `./ast-chunker.ts` performs a genuine module-scope `await Parser.init()` to
// preload tree-sitter WASM grammars, because `AstChunker.chunk()` /
// `.estimate()` are synchronous by contract and `web-tree-sitter`'s init is
// irreducibly async (v0.25.10 exposes no `initSync`). That top-level await is
// legitimate ESM — but it makes the *entire module graph* un-`require()`-able:
//
//     Error [ERR_REQUIRE_ASYNC_MODULE]: require() cannot be used on an ESM
//     graph with top-level await.
//
// `./index.ts` statically re-exports `AstChunker` (and `MixedFormatChunker`,
// which imports it), so ANY import of the package root drags the top-level
// await into the graph. `@adhd/sox-memory-core` compiles to CommonJS and
// `require()`s this package — which crashed it at module load, and also broke
// esbuild bundling under `format: 'cjs'` (`tools/bundle-extension.cjs`).
//
// Nothing in this file imports a chunker, so its graph contains zero top-level
// await. Node's `require(esm)` interop handles a TLA-free ESM module fine, and
// esbuild can inline it into a CJS bundle.
//
// INVARIANT — do not break this:
//   This module MUST NOT import (directly or transitively) from
//   `./ast-chunker.js`, `./mixed-format-chunker.js`, `./chunker-registry.js`,
//   or `./index.js`. A regression test asserts `require()` of the compiled
//   output still succeeds. CJS consumers import `@adhd/sox-ingest/core`;
//   ESM consumers may keep using the package root for the full chunker surface.
