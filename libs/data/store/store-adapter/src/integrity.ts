...
// ── Content-proven-dead -tshm discriminator (BUG-014/DEBT-003) ───────────────

/**
 * Read the `-tshm` coordination header's indexed-frame extent.
 *
 * The `.tshm` is Turso's shared-WAL-index coordination file. Its first 76
 * bytes are a durable snapshot header (`TSHMWAL\0` magic + version 1, the
 * same format in @tursodatabase/database 0.7.1's native and turso
 * 0.7.2's `shared_wal_coordination.rs`). The authoritative "how many frames
 * does this index describe" counter is `max_frame`; empirically (calibrated
 * 2026-08-12 against the real 0.7.1 native, stores with 5..3000 frames) it is
 * recorded at TWO offsets that stay in lockstep with the WAL's current frame
 * count — u64 LE @ 40 and u32 LE @ 56 — and both are zeroed by the engine
 * itself when a connection TRUNCATE-checkpoints the WAL. The probe reads
 * both and reports the larger, so it can only ever OVER-claim the index
 * extent, never under-claim it.
 *
 * This is deliberately a SHALLOW probe: the short-read that breaks opens is
 * produced by stale entries in the frame-index hash blocks (past offset 76),
 * which this probe does not enumerate — so a `maxFrame` of 0 does NOT prove
 * the tshm is healthy (the incident tshm had `max_frame=0` and still
 * short-read at a hash-block offset). The probe only ever says how many
 * frames the index CLAIMS; the caller combines it with the WAL's own extent
 * to decide content-deadness.
 *
 * Deterministic fs-only — no driver open, never writes. Never throws: an
 * absent/unreadable/malformed file yields `readable: false` with a reason.
 */
export interface TshmFrameProbe {
  readable: boolean;
  /** The index's claimed frame extent (max of u64@40 and u32@56). */
  maxFrame: number | null;
  /** Why the probe could not run, when it could not. */
  error?: string;
}

/** The `-tshm` coordination snapshot header is 76 bytes. */
const TSHM_HEADER_BYTE_LEN = 76;
/** Magic at bytes 0..8 of every `.tshm` mapping (turso shared-wal-coordination). */
const TSHM_MAGIC = 'TSHMWAL\0';

export function probeTshmFrameExtent(tshmPath: string): TshmFrameProbe {
  const base: TshmFrameProbe = { readable: false, maxFrame: null };
  let size: number;
  try {
    size = statSync(tshmPath).size;
  } catch (err) {
    return {
      ...base,
      error: `could not stat ${tshmPath}: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  if (size < TSHM_HEADER_BYTE_LEN) {
    return {
      ...base,
      error: `file is ${size} bytes — smaller than the ${TSHM_HEADER_BYTE_LEN}-byte coordination header`,
    };
  }
  const header = Buffer.alloc(TSHM_HEADER_BYTE_LEN);
  let fd: number;
  try {
    fd = openSync(tshmPath, 'r');
  } catch (err) {
    return {
      ...base,
      error: `could not open for read: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  try {
    readSync(fd, header, 0, TSHM_HEADER_BYTE_LEN, 0);
  } catch (err) {
    closeSync(fd);
    return {
      ...base,
      error: `could not read header: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  closeSync(fd);

  if (header.subarray(0, 8).toString('latin1') !== TSHM_MAGIC) {
    return {
      ...base,
      error: `header magic ${JSON.stringify(header.subarray(0, 8).toString('latin1'))} — expected ${JSON.stringify(TSHM_MAGIC)}; not a -tshm file`,
    };
  }
  const version = header.readUInt32LE(8);
  if (version !== 1) {
    return { ...base, error: `unsupported coordination header version ${version} (expected 1)` };
  }
  const maxFrame = Math.max(
    Number(header.readBigUInt64LE(40)),
    header.readUInt32LE(56),
  );
  return { readable: true, maxFrame };
}

/**
 * BUG-014/DEBT-003 — is the `-tshm` beside `dbPath` CONTENT-PROVEN DEAD?
 *
 * The 0.5.7 lease-gate (BUG-009) refuses to reconcile any sidecar while a
 * live peer holds the store — correct for a genuinely fresh tshm, fatal for
 * a stale one: when a close()-TRUNCATE zeroed the WAL but the tshm survived,
 * the ONLY cure is moving the tshm aside, and under live-peer leases the gate
 * declined it while the retry branch burned its budget on a provably
 * non-transient failure (BUG-014, live 5/5 short-read at a frozen offset).
 *
 * The discriminator is CONTENT, never mtime — the mtime heuristic is unsound
 * under multiprocess WAL (the tshm mtime freezes at file creation, so a LIVE
 * index reads as "provably stale"; BUG-007 renamed live tshms with it). A
 * tshm is content-proven dead iff:
 *
 *  1. the `-wal` is absent or 0 bytes — an index of frames a WAL of this size
 *     cannot contain is not being used by anyone (the incident shape, and the
 *     empirically verified safe-under-peer case, triage Probe D); or
 *  2. the WAL is non-empty but the tshm's OWN snapshot claims a frame extent
 *     whose last frame offset lands beyond the WAL EOF — the index describes
 *     frames the WAL does not hold (the Aug-11 shape: tshm indexing frame 511
 *     at offset 2101232 over a 206032-byte WAL).
 *
 * The frame-offset math mirrors the engine's own
 * `frame_offset(frame_id) = 32 + (frame_id − 1) * (24 + page_size)`.
 *
 * Never throws, deterministic fs-only. `dead: false` means "not PROVEN dead"
 * — a genuinely fresh tshm under live peers must keep the BUG-009
 * defer-and-retry behavior (BUG-007 guard), never be reconciled.
 */
export interface TshmContentDeadVerdict {
  dead: boolean;
  /** Why the verdict, for the log/integrity report. Null when not dead. */
  reason: string | null;
}
...
export function isTshmContentDead(dbPath: string): TshmContentDeadVerdict {
  ...
  if (walSize <= 0) {
    return {
      dead: true,
      reason:
        `the -wal is 0 bytes or absent — the -tshm indexes frames a WAL of this size cannot contain, ` +
        `so no live connection can be using it`,
    };
  }

  // 2. WAL non-empty: frame geometry comes from the WAL's own header. A
  //    non-empty WAL with an unreadable header is NOT proven dead — that is
  //    a different failure (corrupt WAL) that moving the tshm cannot cure.
  const walProbe = probeWalFrames(dbPath + '-wal');
  if (!walProbe.readable || walProbe.size === null || walProbe.pageSize === null) {
    return { dead: false, reason: null };
  }

  // 3. The tshm's own claimed extent vs the WAL's capacity.
  const tshm = probeTshmFrameExtent(dbPath + '-tshm');
  if (!tshm.readable || tshm.maxFrame === null || tshm.maxFrame === 0) {
    // Nothing claimed (or no index) — not PROVEN dead; an empty index cannot
    // be the source of a beyond-EOF frame read.
    return { dead: false, reason: null };
  }
  const pageSize = walProbe.pageSize;
  const lastFrameOffset = 32 + (tshm.maxFrame - 1) * (24 + pageSize);
  if (lastFrameOffset > walProbe.size) {
    return {
      dead: true,
      reason:
        `the -tshm indexes frame ${tshm.maxFrame} at offset ${lastFrameOffset}, ` +
        `beyond the ${walProbe.size}-byte -wal EOF — no live connection can be using those frames`,
    };
  }
  return { dead: false, reason: null };
}
 ...
 * CONTENT-proven dead. The default gate (`storeInUse`/quiescence) exists to
 * protect a LIVE index a peer may be reading; a content-dead index (WAL 0
 * bytes/absent, or the index's own snapshot beyond the WAL EOF) is provably
 * not being used by anyone (triage Probe D), so the gate's premise is false.
 * `requireContentDead` re-runs {@link isTshmContentDead} and declines unless
 * the verdict is `dead` — the BUG-007 guard (never rename a live index) is
 * structurally intact even when the quiescence gate is bypassed.
 */
export function recoverStaleWalIndex(
  dbPath: string | undefined,
  opts?: { storeInUse?: boolean; allowUnderLivePeers?: boolean; requireContentDead?: boolean },
): SidecarRecovery {
  ...
  if (opts?.storeInUse === true && opts.allowUnderLivePeers !== true) {
    return {
      attempted: false,
      movedAside: [],
      declined:
  ...
  // (BUG-014/DEBT-003) Content-dead mode: content-deadness is the gate, not
  // quiescence. The caller bypassed the storeInUse decline above; the
  ...
  if (opts?.requireContentDead === true) {
    ...
    if (!verdict.dead) {
      return {
        ...
        movedAside: [],
        declined:
          `refusing to reconcile: the -tshm is NOT content-proven dead — ` +
          `live WAL coordination state may still be in use (BUG-007 guard). ` +
          `Verdict reason: ${verdict.reason ?? 'content-live or unprovable'}.`,
      };
    }
    ...
    try {
      ...
      return {
        ...
        movedAside: [],
        declined:
          `no -tshm sidecar present beside ${dbPath} to reconcile — the content-dead ` +
          `verdict was ${verdict.reason ?? 'unstated'}`,
      };
    }
