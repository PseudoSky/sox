...
- b730340: feat(graph-store): tx-scoped write primitives — `GraphBackend.transaction()` now hands the callback a
  ...
  await graph.transaction(
    async (tx) => {
      const issue = await tx.writeNode("content", {
        kind: "episode",
        name: "i1",
      });
    ...
    { mode: "immediate" }
  ); // BEGIN IMMEDIATE — the ADR-0012 §1 CAS primitive
  ```
...
- 1bc47e2: Add `NodeFilter.isSuperseded`, so a consumer can exclude superseded rows from a read.
...
- 0bbf11e: Surface the stable node UUID as first-class identity. `NodeRecord` gains a
